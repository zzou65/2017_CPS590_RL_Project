...
function net = DPN_Train_NN_2 ( archi, opts, memo, if_use_baseline, n_epoch, n_trajectory, disct_r, max_traj_len)
    AD2D_Global_Vars;
    ...
    for i=1:n_epoch
        ...
        saveloc = sprintf('./DPN_Critic_Controls/Controler_%d_Epoch.mat', i);
...
function [bsize, trainX, trainY] = process_memo_into_trdata(trajbatch, disct_r, if_use_b)
    ...
    for i=1:n_traj
        ...
        for j=1:memo.nstep
            ...
            if(if_use_b == 1)
                %memoY(j, n_out) = (1 + disct_fac) * memo.nnout(j, n_out) - disct_fac * disct_rewd;
