function Lloc = LOAD_Neu_Bnd_PWDG(Edge,Normal,Params,Data,QuadRule,omega,GHandle,varargin)
%LOAD_NEU_BND_PWDG Element load vector for Neumann boundary conditions
%
%   LLOC = LOAD_NEU_BND_PWDG(EDGE,NORMAL,PARAMS,DATA,QUADRULE,OMEGA,GHANDLE)
%   computes the entries of the element load vector corresponding to 
%   Neumann boundary conditions for discontinuous plane waves.
%
...
%     B = B_Vol + B_Imp + B_Dir;
%
%   See also assemLoad_Bnd_PDG2, LOAD_Imp_Bnd_PWDG, LOAD_Dir_Bnd_PWDG, 
%   LOAD_Vol_PWDG, STIMA_Helm_Neu_Bnd_PWDG, STIMA_Helm_Inn_PWDG.
...
%   CH-8092 Zurich, Switzerland
  ...
  %   n = orient*Normal;
    ...
    v0 = i*omega*conj(v) - Params.b/(i*omega)*conj(Dv);
  ...
  % If edge is parametrized
    ...
    for j=1:nDofs
      ...
      VVal = i*omega*conj(ExpVal) - (Params.b/(i*omega))*conj(i*omega*ExpVal.*(N*Dir(j,:).'));
