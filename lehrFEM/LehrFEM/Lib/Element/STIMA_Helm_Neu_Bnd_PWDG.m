function Aloc = STIMA_Helm_Neu_Bnd_PWDG(Edge,Normal,Params,Data,omega,QuadRule,varargin)
%STIMA_HELM_NEU_BND_PWDG Element stiffness matrix for PWDG Helmholtz
%
%   ALOC = STIMA_HELM_NEU_BND_PWDG(EDGE,NOTMAL,PARAMS,DATA,OMEGA)
...
%   STIMA_Helm_Dir_Bnd_PWDG, STIMA_Helm_Inn_PWDG, LOAD_Imp_Bnd_PWDG.
...
%   CH-8092 Zurich, Switzerland
  ...
  %   n = orient*Normal;
    ...
    % Calculate flux (flux of sigma is zero)
    u = v - (Params.b/(i*omega))*Dv;
  ...
  % If edge is parametrized
    ...
    for j=1:nDofs
      for k=1:nDofs
        ...
        U = ExpVal - Params.b*ExpVal.*(N*Dir(j,:).');
        Aloc(k,j) = -i*omega*Aloc(k,j)*sum(QuadRule.w.*(U.*(N*Dir(k,:).')));
      end
    end
