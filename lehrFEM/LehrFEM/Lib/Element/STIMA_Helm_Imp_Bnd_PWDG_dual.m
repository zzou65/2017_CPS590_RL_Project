function Aloc = STIMA_Helm_Imp_Bnd_PWDG_dual(Edge,Normal,Params,varargin)
%STIMA_HELM_IMP_BND_PWDG_DUAL Element stiffness matrix for mixed PWDG
%
%   ALOC = STIMA_HELM_IMP_BND_PWDG_DUAL(EDGE,NORMAL,PARAMS)
%   computes the impedance boundary edge contributions to the element
...
%   n = orient*Normal;
  ...
  % Calculate fluxes
  u = (1-Params.d)*v - Params.d*tau;
  sigma = Params.d*tau - (1-Params.d)*v;
  ...
  Aloc1 = transpose(u(:,ones(3*nDofs,1))).*conj(tau(:,ones(3*nDofs,1)));
  Aloc2 = transpose(sigma(:,ones(3*nDofs,1))).*conj(v(:,ones(3*nDofs,1)));
  Aloc = -Aloc1-Aloc2;
