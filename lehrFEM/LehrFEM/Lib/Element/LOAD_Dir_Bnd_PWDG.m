function Lloc = LOAD_Dir_Bnd_PWDG(Edge,Normal,Params,Data,QuadRule,omega,GHandle,varargin)
%LOAD_DIR_BND_PWDG Element load vector for Dirichlet boundary conditions
%
...
%   Dirichlet boundary conditions for discontinuous plane waves.
%
...
%   to do the integration over the intervall [0,1]:
...
%   vector.
%
%   LLOC = LOAD_DIR_BND_PWDG(...,GPARAM) passes the variable length
...
%     B = B_Vol + B_Imp + B_Dir;
%
%   See also assemLoad_Bnd_PDG2, LOAD_Imp_Bnd_PWDG, LOAD_Neu_Bnd_PWDG, 
%   LOAD_Vol_PWDG, STIMA_Helm_Dir_Bnd_PWDG, STIMA_Helm_Inn_PWDG.
...
%   CH-8092 Zurich, Switzerland
  ...
  %   n = orient*Normal;
    ...
    % Calculate PW test function components
    v0 = Params.a*i*omega*conj(v) - conj(Dv);
  ...
  % If edge is parametrized
    ...
    GVal = GHandle(Gamma,varargin{:});
    Lloc = Params.Geom.dGamma(ones(nDofs,1));
    for j=1:nDofs
      ExpVal = exp(i*omega*(Gamma-X0(ones(nx,1),:))*Dir(j,:).');
      VVal = Params.a*i*omega*conj(ExpVal) - conj(i*omega*ExpVal.*(N*Dir(j,:).'));
