function A = STIMA_Div_x_P1P0(Vertices,ElemInfo,QuadRule,FHandle,varargin)
% STIMA_DIV_X_P1P0 Element stiffness matrix.
%
%   A = STIMA_DIV_X_P1P0(VERTICES,ELEMINFO,QUADRULE,FHANDLE) computes the 
...
%
%   A = STIMA_DIV_X_P1P0(VERTICES,ELEMINFO,QUADRULE,FHANDLE,FPARAM) also
%   handles the additional variable length argument list FPARAM to the
%   function handle FHANDLE.
%
%   Example:
%
%   A = STIMA_Div_x_P1P0([0 0; 1 0; 0 1],0,P7O6(),FHandle);
%
...
%   CH-8092 Zurich, Switzerland
  ...
  A(1) = sum(QuadRule.w.*FVal.*grad_N(:,1))*det_BK;  
  A(2) = sum(QuadRule.w.*FVal.*grad_N(:,3))*det_BK;
  A(3) = sum(QuadRule.w.*FVal.*grad_N(:,5))*det_BK;
