function Aloc = STIMA_Helm_Imp_Bnd_PWDG(Edge,Normal,Params,Data,omega,QuadRule,varargin)
%STIMA_HELM_IMP_BND_PWDG Element stiffness matrix for PWDG Helmholtz
%
%   ALOC = STIMA_HELM_IMP_BND_PWDG(EDGE,NORMAL,PARAMS,DATA,OMEGA)
%   computes the entries of the element stiffness matrix for a
%   discontinuous plane wave discretization of the Helmholtz equation
%   on impedance boundary edges.
%
...
%    D        Scalar coefficient for flux on impedance boundary, should be
%             between 0 and 1.
...
%
%   See also assemMat_Bnd_PDG2, STIMA_Helm_Dir_Bnd_PWDG,
...
%   CH-8092 Zurich, Switzerland
  ...
  %   n = orient*Normal;
    ...
    % Calculate fluxes
    sigma = Params.d/(i*omega)*Dv - (1-Params.d)*v;
    u = (1-Params.d)*v - Params.d/(i*omega)*Dv;
    ...
    Aloc = transpose(u(:,ones(nDofs,1))).*conj(Dv(:,ones(nDofs,1))) ...
      - i*omega*transpose(sigma(:,ones(nDofs,1))).*conj(v(:,ones(nDofs,1)));
  ...
  % If edge is parametrized
    ...
    for j=1:nDofs
      for k=1:nDofs
        ...
        Sigma = Params.d*ExpVal.*(N*Dir(j,:).') - (1-Params.d)*ExpVal;
        U = (1-Params.d)*ExpVal - Params.d*ExpVal.*(N*Dir(j,:).');
        Aloc(k,j) = -i*omega*Aloc(k,j)*sum(QuadRule.w.*(U.*(N*Dir(k,:).') + Sigma));
      end
    end
