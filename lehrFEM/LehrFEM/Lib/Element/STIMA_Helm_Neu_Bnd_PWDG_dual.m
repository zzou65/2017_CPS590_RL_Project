function Aloc = STIMA_Helm_Neu_Bnd_PWDG_dual(Edge,Normal,Params,varargin)
%STIMA_HELM_NEU_BND_PWDG_DUAL Element stiffness matrix for PWDG Helmholtz
% 
%   ALOC = STIMA_HELM_NEU_BND_PWDG_DUAL(EDGE,NORMAL,PARAMS)
%   computes the Neumann boundary edge contributions to the element
...
%   n = orient*Normal;
  ...
  Aloc = -transpose(u(:,ones(3*nDofs,1))).*conj(tau(:,ones(3*nDofs,1)));
