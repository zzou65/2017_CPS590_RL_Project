...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %% JDQR Code obtained from http://www.math.uu.nl/people/sleijpen
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%-------------- display results ----------------------------
if SHOW == 2, MovieTheta, if isempty(FIG), FIG=1; end, figure(FIG), end
...
if SHOW
   ShowChoices(n,nselect,sigma,SCHUR,jmin,jmax,tol,maxit,V,...
         INTERIOR,SHOW,PAIRS,JDV,OLD,lsolver,ls_tol,ls_maxit,ell)
end
...
global L_precond U_precond
 ...
 if ~isempty(L_precond) & ~ischar(L_precond), return, end
 if exist(sigma)==2 & (isempty(L_precond) | isempty(U_precond))
   ok=1; eval('v=feval(sigma,zeros(n,1));','ok=0')
   if ok
     U_precond=L_precond; L_precond=sigma; sigma=[]; 
   end
 end
...
global A_operator type_preconditioner L_precond U_precond P_precond
 ...
 fprintf('\nPRECONDITIONER\n'); tp=type_preconditioner;
  if tp == 0
    fprintf('    No preconditioner\n');
  elseif tp == 1
    fprintf('%13s: ''%s''\n','M',L_precond);
  elseif tp == 2
    fprintf('%13s: ''%s'' [''preconditioner'']\n','M',L_precond);
  elseif tp == 3
    fprintf('%13s = L*U where\n','M');
    fprintf('%13s: ''%s''\n','L',L_precond);
    fprintf('%13s: ''%s''\n','U',U_precond);
  elseif tp == 4
    fprintf('    M = L*U where\n');
    ...
    if tp == 5
      ...
      LU='L*U'; if tp == 7, LU='P\(L*U)'; end
      fprintf('%13s = %s where\n','M',LU);
      fprintf('%13s: [%ix%i %s]\n','L',n,n,Ls);
      Ls='double'; if issparse(U_precond), Ls='sparse'; end
      fprintf('%13s: [%ix%i %s]\n','U',n,n,Ls);
      if tp == 7
        Ls='double'; if issparse(P_precond), Ls='sparse'; end
        fprintf('%13s: [%ix%i %s]\n','P',n,n,Ls);
      end
    end
  end
