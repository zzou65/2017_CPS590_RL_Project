%readdat   read in matrix i matlab coordinate format
%
%usage : A=readdat(filedat)
%
%file = matrix file name
%A    = returned sparse matrix
%
...
function A=readdat(filedat)

% assumes filename includes .dat

fl=length(filedat);
filename=filedat(1:fl-4);
...
eval(['A=spconvert(C);']);
disp(' Successfully finished readdat call');
