function varargout = pgmres_solve(x,A,b,restart,tol,maxit,prec,varargin)
% PGMRES_SOLVE Preconditioned Generalized minimal residuals method. 
%
%   X = PGMRES_SOLVE(X,A,B,RESTART,TOL,MAXIT,PREC) solves the linear system
%   A*X=B from the initial guess X0 using the GMRES method together with
%   the preconditioner PREC. 
%
%   The tolerance TOL and the maximum number of iterations MAXIT are used
%   as stopping criteria for the GMRES iterations.
%
%   [X,FLAG] = PGMRES_SOLVE(X0,A,B,RESTART,TOL,MAXIT,PREC) the integer FLAG
...
%      tolerance TOL.
%
%   [X,FLAG,RELRES] = PGMRES_SOLVE(X0,A,B,RESTART,TOL,MAXIT,PREC) also
%   returns the relative residual RELRES at the last GMRES iteration.
% 
%   [X,FLAG,RELRES,ITER] = PGMRES_SOLVE(X0,A,B,RESTART,TOL,MAXIT,PREC) also
%   returns the number of GMRES iterations that were performed.
%
%   Example:
%
%   prec = @(x,D)x./D;
%   x = pgmres_solve(zeros(size(b)),A,b,10,1e-6,1000,prec,diag(A));
...
%   (ftp netlib2.cs.utk.edu; cd linalg; get templates.ps).
  ...
  if(isa(A,'function_handle'))
    ...
    for iter = 1:maxit
      ...
      r = prec(b-A(x,varargin{:}),varargin{:});
      V(:,1) = r/norm(r);
      s = norm(r)*e1;
      ...
      for i = 1:m
        w = prec(A(V(:,i),varargin{:}),varargin{:});
	    for k = 1:i,
	      H(k,i)= w'*V(:,k);
	      w = w-H(k,i)*V(:,k);
        end
