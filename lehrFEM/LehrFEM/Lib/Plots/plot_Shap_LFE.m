function varargout = plot_Shap_LFE(NMesh,LNumber,Vertex)
% PLOT_SHAP_LFE plot LFE shap functions in 3D with lighting effect
%
%   PLOT_SHAP_LFE(NMESH,LNUMBER,VERTEX) generates a 3D plot of the LFE
%   shape function on the domain represented by Vertex
%
%   The number NMesh determines how fine the mesh is.
%   
%   The number LNumber, which is integer from 1 to 3, determines which LFE
%   shape function to take.
%   
%   The matrix Vertex determine the set of point which represents the
%   destination of the mapping.
%
%   H = PLOT_SHAP_LFE(NMESH,LNUMBER,VERTEX) also returns the handle to the 
%   figure.
%
%   Example:
%
%   plot_Shap_LFE(100,1,[0 0;1 0;0 1]);
...
%   CH-8092 Zurich, Switzerland
    ...
    py1 = zeros(size(px1));
    pz1 = shap_LFE([px1 py1]);
    py2 = linspace(0,1,NMesh)';
    px2 = zeros(size(py2));
    pz2 = shap_LFE([px2 py2]);
    px3 = linspace(0,1,NMesh)';
    py3 = linspace(1,0,NMesh)';
    pz3 = shap_LFE([px3 py3]);
    V1 = affine_map([px1 py1],Vertex);
    V2 = affine_map([px2 py2],Vertex);
    V3 = affine_map([px3 py3],Vertex);
    plot3(V1(:,1),V1(:,2),pz1(:,LNumber),'-k','linewidth',1.5);
    hold on
    plot3(V2(:,1),V2(:,2),pz2(:,LNumber),'-k','linewidth',1.5);
    hold on
    plot3(V3(:,1),V3(:,2),pz3(:,LNumber),'-k','linewidth',1.5);
    hold on
    ...
    [pos dummy1 dummy2] = find(shap(loc,LNumber));
    loc = loc(pos);
    plot3([X(loc) X(loc)] ,[Y(loc) Y(loc)],[zeros(size(loc)) shap(loc,LNumber)],'--k','linewidth',2)
    plot_Shap([X Y],shap(:,LNumber));
