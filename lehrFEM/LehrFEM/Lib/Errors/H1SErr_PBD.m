function err = H1SErr_PBD(Mesh,u,QuadRule,FHandle,varargin)
% H1SERR_PBD Discretization error with respect to H1 semi-norm for
%            quadratic finite elements with parabolic boundary
%            approximation.
%
%   ERR = H1SERR_PBD(MESH,U,QUADRULE,FHANDLE) computes the discretization
...
%
%   ERR = H1SERR_PBD(MESH,U,QUADRULE,FHANDLE,FPARAM) also handles the
  ...
  for i = 1:nElements
    ...
    if(delta > eps)
      ...
      for j = 1:nPts
        ...
        err = err + QuadRule.w(j)*norm(grad_u_EX(j,:)-grad_u_FE)^2*det_Jac;
      end    
      ...
      err = err+sum(QuadRule.w.*sum(abs(grad_u_EX-grad_u_FE).^2,2))*det_BK;
