function err = H1SErr_DGLFE(Mesh,u,QuadRule,FHandle,varargin)
% H1SERR_DGLFE Discretization error measured in the H1 semi-norm for
% discontinuous linear Lagrangian finite elements. 
%
%   ERR = H1SERR_DGLFE(MESH,U,QUADRULE,FHANDLE) computes the discretization
...
%
%   ERR = H1SERR_DGLFE(MESH,U,QUADRULE,FHANDLE,FPARAM) also handles the
%   variable length argument list FPARAM to the exact solution FHANDLE.
%
%   Example:
%
%   err = H1SErr_DGLFE(Mesh,u,QuadRule,grad_Uex);
%
...
%   CH-8092 Zurich, Switzerland
  ...
  grad_N = grad_shap_DGLFE(QuadRule.x);
