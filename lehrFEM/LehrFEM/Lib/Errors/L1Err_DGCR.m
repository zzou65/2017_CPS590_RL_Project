function err = L1Err_DGCR(Mesh,u,QuadRule,FHandle,varargin)
% L1ERR_DGCR Discretization error measured in the L1 norm for discontinuous
% Crouzeix-Raviart finite elements.
%
%   ERR = L1ERR_DGCR(MESH,U,QUADRULE,FHANDLE) computes the discretization
...
%
%   ERR = L1ERR_DGCR(MESH,U,QUADRULE,FHANDLE,FPARAM) also handles the
%   variable length argument list FPARAM to the exact solution FHANDLE.
%
%   Example:
%
%   err = L1Err_DGCR(Mesh,u,QuadRule,Uex);
%
%   See also shap_DGCR.
...
%   CH-8092 Zurich, Switzerland
  ...
  for i = 1:nElements
    ...
    err = err+sum(QuadRule.w.*abs(u_EX-u_FE))*det_BK;
