function err = H1Err_BFE(Mesh,u,QuadRule,FHandle,varargin)
% H1ERR_BFE Discretization error in H1 norm for bilinear finite element
%
%   ERR = H1ERR_BFE(MESH,U,QUADRULE,FHANDLE) computes the discretization
...
%
%   ERR = H1ERR_BFE(MESH,U,QUADRULE,FHANDLE,FPARAM) also handles the
%   variable length argument list FPARAM to the exact solution FHANDLE.
%
%   Example:
%
%   err = H1Err_BFE(Mesh,u,QuadRule,FHandle);
...
%   CH-8092 Zurich, Switzerland
  ...
  for i= 1:nElements
    ...
    for j = 1:nPts
      ...
      err = err + QuadRule.w(j)*(u_EX(j)-u_FE)^2*det_DPhi_K + ...
