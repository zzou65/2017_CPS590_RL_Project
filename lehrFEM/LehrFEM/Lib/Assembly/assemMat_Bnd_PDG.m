function varargout = assemMat_Bnd_PDG(Mesh,nDofs,EHandle,varargin)
% ASSEMMAT_BND_PDG Assemble boundary edge contributions.
%
%   A = ASSEMMAT_BND_PDG(MESH,NDOFS,EHANDLE) assembles the global matrix
...
%
%   A = ASSEMMAT_BND_PDG(MESH,NDOFS,EHANDLE,EPARAM) handles the variable
%   length argument list EPARAM to the function handle EHANDLE during the
%   assembly process. 
%
%   [I,J,A] = ASSEMMAT_BND_PDG(MESH,NDOFS,EHANDLE) assembles the global
...
%    EDGELOC      P-by-3 or N-by-4 matrix connecting egdes to local edges
%                 of elements. 
...
%   Example:
%
%   A = assemMat_Bnd_PDG(Mesh,3,@STIMA_Bnd_DGCR,1,QuadRule, ...
...
%   CH-8092 Zurich, Switzerland
  ...
  I = zeros(nDofs^2*nEdges,1);
  J = zeros(nDofs^2*nEdges,1);
  A = zeros(nDofs^2*nEdges,1);
  ...
  for i = 1:nEdges
    ...
    if(BdFlags(i) < 0)  
      ...
      idx = nDofs*(Data.Element-1)+(1:nDofs);
  ...
  if(nargout > 1)
    ...
    varargout{1} = sparse(I(loc),J(loc),A(loc),nDofs*nElements,nDofs*nElements);  
  end
