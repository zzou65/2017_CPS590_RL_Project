function varargout = assemMat_Inn_DG(Mesh,EHandle,varargin)
% ASSEMMAT_INN_DG Assemble discontinuous Crouzeix-Raviart FE interior edge
% contributions.
%
%   A = ASSEMMAT_INN_DG(MESH,EHANDLE) assembles the global matrix from the
%   local element contributions given by the function handle EHANDLE and
%   returns the matrix in a sparse representation.
%
%   A = ASSEMMAT_INN_DG(MESH,EHANDLE,EPARAM) handles the variable length
%   argument list EPARAM to the function handle EHANDLE during the assembly
%   process. 
%
%   [I,J,A] = ASSEMMAT_INN_DG(MESH,EHANDLE) assembles the global matrix
...
%   Example:
%
%   A = assemMat_Inn_DG(Mesh,@STIMA_Inn_DGCR);
%  
...
%   CH-8092 Zurich, Switzerland
  ...
  for i = 1:nEdges
    ...
    if(BdFlags(i) >= 0)
      ...
      idx_l = 3*(LData.Element-1)+[1 2 3]; 
      idx_r = 3*(RData.Element-1)+[1 2 3];
      idx = [idx_l idx_r];
