function L = assemLoad_Bnd_PDG(Mesh,nDofs,EHandle,varargin) 
% ASSEMLOAD_BND_PDG Assemble boundary edge contributions.
%
%   A = ASSEMLOAD_BND_PDG(MESH,NDOFS,EHANDLE) assembles the global load
%   vector from the local element contributions given by the function
%   handle EHANDLE. The integer NDOFS speicifies the number of degrees of
%   freedom per element.
%
%   A = ASSEMLOAD_BND_PDG(MESH,NDOFS,EHANDLE,EPARAM) handles the variable
...
%   Lbnd = assemLoad_Bnd_PDG(Mesh,3,@LOAD_Bnd_DGCR,1,gauleg(0,1,2), ...
%                            @shap_DGCR,@grad_shap_DGCR,sigma,uD);
