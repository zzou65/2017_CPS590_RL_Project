function L = assemNeu_BFE(Mesh,BdFlags,L,QuadRule,FHandle,varargin)
% ASSEMNEU_BFE Neumann boundary conditions.
%
%   L = ASSEMNEU_BFE(MESH,BDFLAG,L,QUADRULE,FHANDLE) incoporates the
...
%
%   L = ASSEMNEU_BFE(MESH,BDFLAG,L,QUADRULE,FHANDLE,FPARAM) also handles
...
%   Example:
%
%   L = assemNeu_BFE(Mesh,BdFlags,L,QuadRule,FHandle);
%
%   See also GET_BDEDGES, SHAP_BFE.
...
%   CH-8092 Zurich, Switzerland
  ...
  N = shap_BFE([QuadRule.x zeros(nGauss,1)]);
  ...
  for j1 = BdFlags
    ...
    for j2 = Loc'
      ...
      if(Mesh.Edge2Elem(j2,1))
        ...
        id_s = Mesh.Elements(Elem,EdgeLoc);
        id_e = Mesh.Elements(Elem,rem(EdgeLoc,4)+1);
