function err = H1SErrDistr_BFE(Mesh,u,QuadRule,FHandle,varargin)
% H1SERRDISTR_BFE Discretization error distribution in H1 semi-norm for 
% bi-linear finite elements.
%
%   ERR = HISERRDISTR_BFE(MESH,U,QUADRULE,FHANDLE) computes the discretization
...
%
%   ERR = HISERRDISTR_BFE(MESH,U,QUADRULE,FHANDLE,FPARAM) also handles the
%   variable length argument list FPARAM to the exact solution FHANDLE.
%
%   Example:
%
%   err = HISErrDistr_BFE(Mesh,u,QuadRule,FHandle);
...
%   CH-8092 Zurich, Switzerland
  ...
  for i= 1:nElements
    ...
    for j = 1:nPts
      ...
      TErr = TErr + QuadRule.w(j)*sum((grad_u_EX(j,:)-grad_u_FE).^2)*det_DPhi_K;
