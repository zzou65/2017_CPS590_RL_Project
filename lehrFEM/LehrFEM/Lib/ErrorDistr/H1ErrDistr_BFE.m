function err = H1ErrDistr_BFE(Mesh,u,QuadRule,FHandle,varargin)
% H1ERRDISTR_BFE discretization error distribution in H1 norm for bilinear 
% finite element
%
%   ERR = H1ERRDISTR_BFE(MESH,U,QUADRULE,FHANDLE) computes the discretization
...
%
%   ERR = H1ERRDISTR_BFE(MESH,U,QUADRULE,FHANDLE,FPARAM) also handles the
%   variable length argument list FPARAM to the exact solution FHANDLE.
%
%   Example:
%
%   err = H1ErrDistr_BFE(Mesh,u,QuadRule,FHandle);
...
%   CH-8092 Zurich, Switzerland
  ...
  for i= 1:nElements
    ...
    for j = 1:nPts
      ...
      u_FE = u(vidx(1))*N(j,1)+u(vidx(2))*N(j,2) + ...
             u(vidx(3))*N(j,3)+u(vidx(4))*N(j,4);
      grad_u_FE = (u(vidx(1))*grad_N(j,1:2)+u(vidx(2))*grad_N(j,3:4) + ...
                   u(vidx(3))*grad_N(j,5:6)+u(vidx(4))*grad_N(j,7:8))*inv_DPhi_K;
    
      TErr = TErr + QuadRule.w(j)*(u_EX(j)-u_FE)^2*det_DPhi_K + ...
                    QuadRule.w(j)*sum((grad_u_EX(j,:)-grad_u_FE).^2)*det_DPhi_K;
