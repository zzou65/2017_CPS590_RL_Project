...
%  Err_REC_10.eps,  Err_RES_10.eps, Err_HIER_10.eps,
%  Err_REC_20.eps,  Err_RES_20.eps, Err_HIER_20.eps,
...
%   CH-8092 Zurich, Switzerland
  ...
  NITER = 21; %38;                    % Total number of iterations
  NREFS = 8;                     % Number of red refinement steps
  PLOTS = [ 10 20];              % Plots of error distributions       
  ...
  for iter = 1:NITER
    ...
    Err_K = H1SErrDistr_LFE(Mesh,U,P7O6(),UEX_HANDLE);
    ...
    if(~isempty(find(iter == PLOTS)))
        ...
        % Generate figure for EST
        
        fig = plot_EST(Eta_K,Mesh);
        colorbar;
        title(['{\bf Residual based error estimate, adaptive refinement level '...
	       int2str(iter) ' }']);
...
%        system(['gv ' FileName ' &']);
        ...
        % Generate figure for ERR
        
        fig = plot_EST(Err_K,Mesh);
        colorbar;
        title(['{\bf Energy rrror distribution, adaptive refinement level ' int2str(iter) ' (RES)}']);
        xlabel(['{\bf' sprintf('Err = %2.2e',Err(iter)) '}']);
        text(.1,-.4,'# Coordinates','FontWeight','bold');
        text(.6,-.4,':','FontWeight','bold');
        text(.7,-.4,int2str(size(Mesh.Coordinates,1)),'FontWeight','bold');
        text(.1,-.55,'# Elements','FontWeight','bold');
        ...
        FileName = ['Err_RES_' int2str(iter) '.eps'];
        print('-depsc', FileName);
%        close(fig);
%        system(['gv ' FileName ' &']);
  ...
  for iter = 1:NITER
    ...
    if(~isempty(find(iter == PLOTS)))
        ...
        % Generate figure for ErrEst
        
        fig = plot_EST(Eta_K,Mesh);
        colorbar;
        title(['{\bf Recovery based estimate, adaptive refinement level '...
	       int2str(iter) ' }']);
        xlabel(['{\bf' sprintf('Eta = %2.2e',Eta(iter)) '}']);
        text(.1,-.4,'# Coordinates','FontWeight','bold');
        text(.6,-.4,':','FontWeight','bold');
        ...
        fig = plot_EST(Err_K,Mesh);
        colorbar;
        title(['{\bf Energy error distribution, adaptive refinement level '...
	       int2str(iter) ' (REC)}']);
        xlabel(['{\bf' sprintf('Err = %2.2e',Err(iter)) '}']);
        text(.1,-.4,'# Coordinates','FontWeight','bold');
        text(.6,-.4,':','FontWeight','bold');
        ...
        FileName = ['Err_REC_' int2str(iter) '.eps'];
        print('-depsc', FileName);
        close(fig);
        system(['gv ' FileName ' &']);
  ...
  for iter = 1:NITER
    ...
    if(~isempty(find(iter == PLOTS)))
        ...
        fig = plot_EST(Eta_K,Mesh);
        colorbar;
        title(['{\bf Hierarchical error estimate, adaptive refinement level '...
	       int2str(iter) ' }']);
        xlabel(['{\bf' sprintf('Eta = %2.2e',Eta(iter)) '}']);
        text(.1,-.4,'# Coordinates','FontWeight','bold');
        text(.6,-.4,':','FontWeight','bold');
        ...
        FileName = ['Err_HIER_' int2str(iter) '.eps'];
        print('-depsc', FileName);
        close(fig);
        system(['gv ' FileName ' &']);
  ...
  fig = figure();
  plot(Data.nDofs(:,1),Data.Err(:,1),'-x', ...
       Data.nDofs(:,2),Data.Err(:,2),'-*', ...
       Data.nDofs(:,3),Data.Err(:,3),'->', ...
       Data.nDofs_RED,Data.Err_RED,'-o');   
  grid('on');
  set(gca,'XScale','log','YScale','log');
  title('{\bf Convergence rates for energy error}');
  xlabel('{\bf Dofs [log]}');
  ylabel('{\bf Errors [log]}');
  legend([ Data.LegendName{1} ' based ad. ref.'], ...
         [ Data.LegendName{2} ' based ad. ref.'], ...
         [ Data.LegendName{3} ' based ad. ref.'], ...
         'Reg. ref.', ...
         'Location','NorthEast');
