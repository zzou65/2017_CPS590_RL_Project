...
%   CH-8092 Zurich, Switzerland
    ...
    for iter = 1:NREFS
        ...
        if( iter == NREFS )
            ...
            plot_LFE(V(:,NEigen-1),Mesh);
            colorbar;
            title(['{\bf Solution:1 '                             ...  
                   'eigenvalue =' num2str(d(NEigen-1,NEigen-1),4) ... 
                   ', meshwidth = ' num2str(M_W(iter),3) '}' ]);
            print('-depsc', 'Eig_func1_LShap_LFE.eps');
            !gv Eig_func1_LShap_LFE.eps &            
            
            plot_LFE(V(:,NEigen-2),Mesh);
            colorbar;
            title(['{\bf Solution:2 '                             ...  
                   'eigenvalue =' num2str(d(NEigen-2,NEigen-2),4) ... 
                   ', meshwidth = ' num2str(M_W(iter),3) '}' ]);
            print('-depsc', 'Eig_func2_LShap_LFE.eps');
            !gv Eig_func2_LShap_LFE.eps &
    ...
    title('\bfConvergence rate for eigenvalue errors(L-shape) LFE');
    xlabel('\bfh');
    ...
    print('-depsc', 'rate_Eig_LShap_LFE.eps');
    !gv rate_Eig_LShap_LFE.eps &
