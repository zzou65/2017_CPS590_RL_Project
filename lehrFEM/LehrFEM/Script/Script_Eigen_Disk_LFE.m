...
%   CH-8092 Zurich, Switzerland 
    ...
    for iter = 1:NRef 
        ...
        if ( iter == NRef )
            ...
            plot_LFE(U(:,NEigen),Mesh);
            colorbar;
            title(['{\bf Solution: 1 '  '  eigenvalue = '...
                num2str(d(NEigen,NEigen))  ', meshwidth = ' num2str(M_W(iter)) '}' ]);
            print('-depsc', 'Eig_func1_disk_LFE.eps');
            !gv Eig_func1_disk_LFE.eps &

            plot_LFE(U(:,NEigen-1),Mesh);
            colorbar;
            title(['{\bf Solution: 2 '  '  eigenvalue = '...
                num2str(d(NEigen-1,NEigen-1))  ', meshwidth = ' num2str(M_W(iter)) '}' ]);
            print('-depsc', 'Eig_func2_disk_LFE.eps');
            !gv Eig_func2_disk_LFE.eps &
    ...
    title('\bfConvergence rate for eigenvalue errors(Disk) LFE');
    xlabel('\bfh');
    ...
    print('-depsc', 'rate_Eig_disk_LFE.eps');
    !gv rate_Eig_disk_LFE.eps &
