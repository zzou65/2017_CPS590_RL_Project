...
% CH-8092 Zurich, Switzerland
  ...
  G_HANDLE = @(x,varargin)zeros(size(x,1),1);  % Dirichlet boundary data
  ...
  clear('BPX_MASS','M');
  caxis([-.1 1.1])
  plot_LFE(U_old,CMesh);
  title(['\bf\fontsize{10}\theta=',num2str(THETA),'   No.0 t=0'])
  set(gca,'CameraPosition', [-1 -1 1]*10,...
        'CameraTarget',[.25 .25 0], ...
        'CameraUpVector',[0 0 1], ...
        'CameraViewAngle',10, ...
        'DataAspectRatio', [1 1 1],...
        'Position',[0 -.2 1 1])
  print('-depsc',['dcont_TH',num2str(THETA*10),'_0.eps'])
  ...
  for i = 1:NSTEPS
    ...
    if(~mod(i/5,1) | i<10)
        ...
        caxis([-.1 1.1])
        title(['\bf\fontsize{10}\theta=',num2str(THETA),'   No.',num2str(i),'  t=',num2str(i*T/NSTEPS)]);
        set(gca,'CameraPosition', [-1 -1 1]*10,...
            'CameraTarget',[.25 .25 0], ...
            'CameraUpVector',[0 0 1], ...
            'CameraViewAngle',10, ...
            'DataAspectRatio', [1 1 1],...
            'Position',[0 -.2 1 1])
        FileName = ['dcont_TH',num2str(THETA*10),'_',num2str(i),'.eps']
