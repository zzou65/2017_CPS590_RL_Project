...
switch n
    ...
    case 1 % easy smooth
     ...
     case 4  % Houston, Sueli, Schwab hp-FEM... 2002
      ...
      case 5 % Mathiess Dipl
        ...
        data.U_EX_Handle=@(x,d,a)...
            ...
            +exp((2*(x(:,1)-1)+3*(x(:,2)-1))/a);
        % solution grad u
        data.GRAD_U_EX_Handle=...
            @(x,d,a,varargin)[2/a*exp((2*(x(:,1)-1) + 3*(x(:,2)-1))/a) - exp(3*(x(:,2)-1)/a)+...
            x(:,2).^2-2/a*x(:,2).^2.*exp(2*(x(:,1)-1)/a),...                     
            x(:,1).*x(:,2)*2 - 2*x(:,2).*exp(2*(x(:,1)-1)/a) - 3/a*x(:,1).*exp(3*(x(:,2)-1)/a)+...
            3/a*exp((2*(x(:,1)-1)+3*(x(:,2)-1))/a) ];
        % -a*div grad u+b*v grad u
        data.SOL_Handle=@(x,elemflag,a,b)...
     ...
     case 6 %  Franca  CMAME 92
        ...
        % solution u
        data.U_EX_Handle=...
            @(x,varargin)sin(pi*x(:,1)).*sin(pi*x(:,2));
        % solution grad u
        data.GRAD_U_EX_Handle=...
            @(x,varargin)pi*[cos(pi*x(:,1)).*sin(pi*x(:,2)) ...  % Gradient of exact solution
                              sin(pi*x(:,1)).*cos(pi*x(:,2))];
        data.V_GRAD_U_EX_Handle=...
            @(x,varargin)pi*(cos(pi*x(:,1)).*sin(pi*x(:,2))+... % Gradient of exact solution
                              sin(pi*x(:,1)).*cos(pi*x(:,2)));                  
        % -a*div grad u+b*v grad 
        data.SOL_Handle=...
            @(x,elemflag,a)a*2*pi^2*sin(pi*x(:,1)).*sin(pi*x(:,2))+...
        ...
        % solution u
        data.U_EX_Handle=...
            @(x,varargin) x(:,1).^n+x(:,2).^n;
        % solution w
        data.W_EX_Handle=...
            @(x,varargin) x(:,1).*x(:,2);
        % solution grad u
        data.GRAD_U_EX_Handle=...
            @(x,varargin)n*[x(:,1).^(n-1) x(:,2).^(n-1)]; 
        data.V_GRAD_U_EX_Handle=...
            @(x,varargin)n*(x(:,1).^(n-1)+x(:,2).^(n-1));                  
        % -a*div grad u+b*v grad
        data.SOL_Handle=...
            @(x,elemflag,a) -a*n*(n-1)*(x(:,1).^(n-2)+x(:,2).^(n-2))...
                                    +n*(x(:,1).^(n-1)+x(:,2).^(n-1));
        data.Coordinates=[-1 -1; 1 -1; 1 1;-1 1];
        data.Elements=[1 2 4; 2 3 4];
        data.boundtype=[-1 -1 -1 -1];
