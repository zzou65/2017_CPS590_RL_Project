function Aloc = STIMA_InfSup_HP(Vertices,ElemInfo,EDofs,EDir,CDofs,QuadRule,Shap,VHandle,varargin)
...
%   CH-8092 Zurich, Switzerland
  ...
  T_inv_BK=transpose(inv_BK);
  
  v=VHandle(QuadRule.x*BK + ones(size(QuadRule.x,1),1)*bK);
  w=QuadRule.w*det_BK;
  
  for i = 1:3
    grad_shap_1 = sum(v.*(Shap.vshap.grad_shap{i}*T_inv_BK),2);  
      
    % Vertex vs vertex
      
    for j = i:3
      grad_shap_2 = sum(v.*(Shap.vshap.grad_shap{j}*T_inv_BK),2);
      Aloc(i,j) = sum(w.*grad_shap_1.*grad_shap_2);
    end
    ...
    offset = 3;
    for j = 1:EDofs(1)
      grad_shap_2 = EDir(1)^(j+1)*sum(v.*(Shap.eshap{1}.grad_shap{j}*T_inv_BK),2);
      Aloc(i,j+offset) = sum(w.*grad_shap_1.*grad_shap_2);
    end
    ...
    offset = 3+EDofs(1);
    for j = 1:EDofs(2)
      grad_shap_2 = EDir(2)^(j+1)*sum(v.*(Shap.eshap{2}.grad_shap{j}*T_inv_BK),2);
      Aloc(i,j+offset) =sum(w.*grad_shap_1.*grad_shap_2);    
    end
    ...
    for j = 1:EDofs(3)
      grad_shap_2 = EDir(3)^(j+1)*sum(v.*(Shap.eshap{3}.grad_shap{j}*T_inv_BK),2);
      Aloc(i,j+offset) = sum(w.*grad_shap_1.*grad_shap_2);
    end
    ...
    offset = 3+sum(EDofs);
    for j = 1:CDofs
      grad_shap_2 = sum(v.*(Shap.cshap.grad_shap{j}*T_inv_BK),2);
      Aloc(i,j+offset) = sum(w.*grad_shap_1.*grad_shap_2);
    end
  ...
  offset_1 = 3;
  for i = 1:EDofs(1)
    grad_shap_1 = EDir(1)^(i+1)*sum(v.*(Shap.eshap{1}.grad_shap{i}*T_inv_BK),2);  
    ...
    for j = i:EDofs(1)
      grad_shap_2 = EDir(1)^(j+1)*sum(v.*(Shap.eshap{1}.grad_shap{j}*T_inv_BK),2);  
      Aloc(i+offset_1,j+offset_2) = sum(w.*grad_shap_1.*grad_shap_2);
    end
  ...
  offset_1 = 3+EDofs(1);
  for i = 1:EDofs(2)
    grad_shap_1 = EDir(2)^(i+1)*sum(v.*(Shap.eshap{2}.grad_shap{i}*T_inv_BK),2);  
  ...
  for i = 1:EDofs(3)
    grad_shap_1 = EDir(3)^(i+1)*sum(v.*(Shap.eshap{3}.grad_shap{i}*T_inv_BK),2);  
  ...
  offset = 3+sum(EDofs);
  for i = 1:CDofs
    grad_shap_1 = sum(v.*(Shap.cshap.grad_shap{i}*T_inv_BK),2);      
    for j = i:CDofs
      grad_shap_2 = sum(v.*(Shap.cshap.grad_shap{j}*T_inv_BK),2);
      Aloc(i+offset,j+offset) = sum(w.*grad_shap_1.*grad_shap_2);
    end
  end
