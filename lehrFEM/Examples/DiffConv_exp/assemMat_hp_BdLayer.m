function varargout = assemMat_hp_BdLayer(Mesh,Elem2Dof,EHandle,varargin)
...
%   CH-8092 Zurich, Switzerland
  ...
  nCoordinates = size(Mesh.Coordinates,1);  % Number of elements
  ...
  for i = 1:nElements
    ...
    edges = nonzeros(Mesh.Vert2Edge(vidx,:));
    if  (sum(Mesh.BdFlags(edges,:)==-2)==0)
  ...
  if(nargout > 1)
    varargout{1} = I(1:range);
    varargout{2} = J(1:range);
    varargout{3} = A(1:range);
  else
    s=nCoordinates+Elem2Dof.tot_EDofs+Elem2Dof.tot_CDofs;  
    varargout{1} = sparse(I(1:range),J(1:range),A(1:range),s,s);      
  end
