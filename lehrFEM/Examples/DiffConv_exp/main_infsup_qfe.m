...
%   M_QFE_v=assemMat_QFE(NewMesh,@MASS_QFEquad,P3O2());
   M_QFE_m=assemMat_QFE(NewMesh,@MASS_QFEquad,P7O4());
...
%    E_2v = M_QFE(FreeDofs,FreeDofs)\A_2v(FreeDofs,FreeDofs); E_2v=A_2v(FreeDofs,FreeDofs)'*E_2v;
   E_2m = M_QFE(FreeDofs,FreeDofs)\A_2m(FreeDofs,FreeDofs); E_2m=A_2m(FreeDofs,FreeDofs)'*E_2m;
...
%   err(i,5) = 1/eigs(inv(full(E_2m))*B(FreeDofs,FreeDofs),[],1,'LM');
   err(i,4) = 1/max(eig(inv(full(E_2m))*full(B(FreeDofs,FreeDofs))));
...
%    err(i,9) = 1/eigs(inv(full(E_supg))*B(FreeDofs,FreeDofs),[],1,'LM');
  ...
  print -depsc './plots/infsup_QFE.eps'
...
%   
%   print -deps 'infsup0formsI2_qfe_eps.eps'
