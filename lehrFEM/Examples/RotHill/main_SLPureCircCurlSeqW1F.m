function main_SLPureCircCurlRateW1F(NREFS, CFLs, T1, locIntegrate, Problem)
...
for j=1:NREFS
    ...
    for k = 1:size(CFLs,2)
        ...
        %H_old=[H_old];
        A_old = assemCochain_0f(NewMesh,A_Handle,gauleg(0,1,10),0);
        H_old=TopGrad*A_old;
        ...
        % timestepping
        time = zeros(steps(j,k),1);
        errCurl = zeros(steps(j,k),1);
        time(1) = 0;
        errCurl(1)= HCurlSErr_W1F(NewMesh,H_old,P7O6(),@(x,varargin)zeros(size(x,1),1),0,0);
        ...
        for i = 1:steps(j,k)
            ...
            errCurl(i+1) = HCurlSErr_W1F(NewMesh,H_old,P7O6(),@(x,varargin)zeros(size(x,1),1),0,0);
        end % timestep
        
        fid = fopen(['./results/BumpCurlSeq_',locIntegrate,'_',Problem,'_',num2str(T1),'_',num2str(CFLs(k)),'_',num2str(h(j)),'.txt'], 'wt');
        fprintf(fid,'tau ');
        fprintf(fid, ' Curl ');
        fprintf(fid, '\n');
        for i = 1:(steps(j,k)+1)
            fprintf(fid, '%12.8e ', [time(i), errCurl(i)]);
            fprintf(fid, '\n');
        end
        fprintf(fid, '\n');
        fclose(fid)
        ...
        %error(j,k) = HCurlSErr_W1F(NewMesh,H_old,P7O6(),@(x,varargin)zeros(size(x,1),1),0,0);
    end %CFLS
end % mesh
