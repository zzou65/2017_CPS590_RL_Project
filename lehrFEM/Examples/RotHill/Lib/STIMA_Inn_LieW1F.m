function Aloc = STIMA_Inn_LieW1F(Edge,Normal,BdFlag,LData,RData,V_Handle, QuadRule,varargin)
%
...
%   CH-8092 Zurich, Switzerland
  ...
  V_NL1=sum(NL1.*Fval,2);
  V_NL2=sum(NL2.*Fval,2);
  V_NL3=sum(NL3.*Fval,2);
  ...
  V_NR1=sum(NR1.*Fval,2);
  V_NR2=sum(NR2.*Fval,2);
  V_NR3=sum(NR3.*Fval,2);
  ...
  Aloc(1,1) = gamma*sum(QuadRule.w.*N_NL1.*V_NL1);
  Aloc(1,2) = gamma*sum(QuadRule.w.*N_NL1.*V_NL2);
  Aloc(1,3) = gamma*sum(QuadRule.w.*N_NL1.*V_NL3);
  
  Aloc(1,4) = gamma*sum(QuadRule.w.*N_NL1.*V_NR1);
  Aloc(1,5) = gamma*sum(QuadRule.w.*N_NL1.*V_NR2);
  Aloc(1,6) = gamma*sum(QuadRule.w.*N_NL1.*V_NR3);

  Aloc(2,1) = gamma*sum(QuadRule.w.*N_NL2.*V_NL1);
  Aloc(2,2) = gamma*sum(QuadRule.w.*N_NL2.*V_NL2);
  Aloc(2,3) = gamma*sum(QuadRule.w.*N_NL2.*V_NL3);
  
  Aloc(2,4) = gamma*sum(QuadRule.w.*N_NL2.*V_NR1);
  Aloc(2,5) = gamma*sum(QuadRule.w.*N_NL2.*V_NR2);
  Aloc(2,6) = gamma*sum(QuadRule.w.*N_NL2.*V_NR3);

  Aloc(3,1) = gamma*sum(QuadRule.w.*N_NL3.*V_NL1);
  Aloc(3,2) = gamma*sum(QuadRule.w.*N_NL3.*V_NL2);
  Aloc(3,3) = gamma*sum(QuadRule.w.*N_NL3.*V_NL3);
  
  Aloc(3,4) = gamma*sum(QuadRule.w.*N_NL3.*V_NR1);
  Aloc(3,5) = gamma*sum(QuadRule.w.*N_NL3.*V_NR2);
  Aloc(3,6) = gamma*sum(QuadRule.w.*N_NL3.*V_NR3);

  Aloc(4,1) = -gamma*sum(QuadRule.w.*N_NR1.*V_NL1);
  Aloc(4,2) = -gamma*sum(QuadRule.w.*N_NR1.*V_NL2);
  Aloc(4,3) = -gamma*sum(QuadRule.w.*N_NR1.*V_NL3);
  
  Aloc(4,4) = -gamma*sum(QuadRule.w.*N_NR1.*V_NR1);
  Aloc(4,5) = -gamma*sum(QuadRule.w.*N_NR1.*V_NR2);
  Aloc(4,6) = -gamma*sum(QuadRule.w.*N_NR1.*V_NR3);

  Aloc(5,1) = -gamma*sum(QuadRule.w.*N_NR2.*V_NL1);
  Aloc(5,2) = -gamma*sum(QuadRule.w.*N_NR2.*V_NL2);
  Aloc(5,3) = -gamma*sum(QuadRule.w.*N_NR2.*V_NL3);
  
  Aloc(5,4) = -gamma*sum(QuadRule.w.*N_NR2.*V_NR1);
  Aloc(5,5) = -gamma*sum(QuadRule.w.*N_NR2.*V_NR2);
  Aloc(5,6) = -gamma*sum(QuadRule.w.*N_NR2.*V_NR3);

  Aloc(6,1) = -gamma*sum(QuadRule.w.*N_NR3.*V_NL1);
  Aloc(6,2) = -gamma*sum(QuadRule.w.*N_NR3.*V_NL2);
  Aloc(6,3) = -gamma*sum(QuadRule.w.*N_NR3.*V_NL3);
  
  Aloc(6,4) = -gamma*sum(QuadRule.w.*N_NR3.*V_NR1);
  Aloc(6,5) = -gamma*sum(QuadRule.w.*N_NR3.*V_NR2);
  Aloc(6,6) = -gamma*sum(QuadRule.w.*N_NR3.*V_NR3);
