% Run script for bilinear finite elements.
...
% CH-8092 Zurich, Switzerland
  ...
  F_HANDLE = @(x,varargin)-6*x(:,1);                                                                  % Righ hand-side source term
  GD_HANDLE = @(x,varargin)x(:,1).^2+x(:,2).^2;                                                       % Dirichlet boundary data
  SIGMA_HANDLE = @(x,varargin)x(:,1);                                                                 % Heat conductivity
  ...
  A = assemMat_BFE(Mesh,@STIMA_Heat_BFE,QuadRule,SIGMA_HANDLE);
