% Run script for linear advection problem convergence plot for smooth initial data.

% linear SSP Runge Kutta (GottliebShuTadmor2001 SIAM Review)

% Copyright 2006-2007 Patrick Meury Holger Heumann
% SAM - Seminar for Applied Mathematics
% ETH-Zentrum
% CH-8092 Zurich, Switzerland
    
  % Initialize constants
  X0 =0;                 % Left end point of interval
  X1 =2;                 % Right end point of interval
  NPOINTS =10.*[1:10];   % Initial set of grid points
  P=[0:6];                 % ploynomial degree 
  CFL=0.5*1./(2*P+1);
% CFL=0.1;
  T =2;                  % Final time
%   
%    U0 = @(x,t)(mod(x,2)>0.5).*(x<1.5);                 % Initial data
%    U_ex =@(x,t)(mod(0.5+t,X1)<mod(1.5+t,X1))*...
%                       ((x>mod(0.5+t,X1)).*(x<mod(1.5+t,X1)))+...
%                (mod(0.5+t,X1)>mod(1.5+t,X1))*...
%                       ((x>mod(0.5+t,X1)).*(x<X1)+(x>0).*(x<mod(1.5+t,X1)));      % Initial data
      U0 = @(x,t)1+0.2*sin(pi*x);                % Initial data
      U_ex =@(x,t)1+0.2*sin(pi*(x-t));           % Initial data
  G = @(x,t)zeros(size(x,1),1);                 % Right hand side load data
  FLUX = @(x) x;                                % Flux function
  NUMFLUX = @(x,y) (x+y)/2;                     % Numerical flux function
%  NUMFLUX = @(x,y) x;                     % Numerical flux function
  %ULIM = @SlopeLim_lr;                          % Limiter for ansatz functions
  ULIM = @(c,x,varargin)x;                      % no limiter
  %SULIM=@(c,x,varargin)x;
  
  % container for error calculation
  err0=zeros(size(NPOINTS,2),size(P,2));
  h=err0;
  err1=err0;
  err2=err0;
  
  % quadrature rule and shape functions at quadrature point and boundary
  for q=1:size(P,2)
      
      % Initialize TVD scheme (P+1-stage Runge-Kutta scheme)
 
      NSTAGES = min([P(q)+1 3]);
      %NSTAGES=3;
      A_RK=RKDG_gamma(NSTAGES);
        
      % quadrature rule and shape functions at quadrature point and boundary
      QuadRule=gauleg(-1,1,ceil(P(q)+3/2));
      Shap=shap_Leg_1D(QuadRule.x,P(q));
      Grad_Shap=grad_shap_Leg_1D(QuadRule.x,P(q));
      Inn_shap=shap_Leg_1D([-1;1],P(q));
      Inn_gradshap=grad_shap_Leg_1D([-1;1],P(q));
  
      % error calculation
      qr=gauleg(-1,1,ceil(P(q)+3/2)*5);
      err_Shap=shap_Leg_1D(qr.x,P(q));
  
      for s=1:size(NPOINTS,2);
      
          % Initialize mesh
      
          Coordinates = X0 + (X1-X0)/(NPOINTS(s)-1)*(0:(NPOINTS(s)-1));

          % Initialize polynomial degrees

          p=P(q)*ones(1,NPOINTS(s)-1);
  
          %Assemble mass matrix and extract diagonal
          M=assemMat_Vol_hpDG_1D(Coordinates,p,@MASS_Vol_hpDG_1D,QuadRule,Shap);
          %M=spdiags(M,0);
  
          % Compute initial data
  
          L = assemLoad_hpDG_1D(Coordinates,p,QuadRule,Shap,U0,0);
          U = M\L;
  
          % Integrate ODE system
     
          dx=min(Coordinates(2:end)-Coordinates(1:end-1));
          dt=CFL(q)*dx;
          NSTEPS=ceil(T/dt);
          Tend=NSTEPS*dt;
           
          tau = zeros(1,NSTAGES+1);
          L = zeros(size(U,1),NSTAGES+1);
          V = zeros(size(U,1),NSTAGES+1);
          PV = zeros(size(U,1),NSTAGES+1);
          
          for i = 1:NSTEPS 
             
             V(:,1) = U;
             Bvol=assemRKDG_update_Vol(Coordinates,V(:,1),p,Shap,Grad_Shap,FLUX,QuadRule);
             Binn=assemRKDG_update_Inn(Coordinates,V(:,1),p,Inn_shap,NUMFLUX);
             L(:,1) = M\(Bvol-Binn);
            
             V(:,NSTAGES+1)=A_RK(1)*V(:,1);
             
             for j = 2:(NSTAGES)
                V(:,j)=V(:,j-1)+dt*L(:,j-1);
                
                V(:,NSTAGES+1)=V(:,NSTAGES+1)+A_RK(j)*V(:,j) ;
                
                Bvol=assemRKDG_update_Vol(Coordinates,V(:,j),p,Shap,Grad_Shap,FLUX,QuadRule);
                Binn=assemRKDG_update_Inn(Coordinates,V(:,j),p,Inn_shap,NUMFLUX);
                L(:,j) = M\(Bvol-Binn);
             
             end
             V(:,NSTAGES+1)=V(:,NSTAGES+1)+A_RK(NSTAGES)*dt*L(:,NSTAGES);
                           
             % Update solution
             U = V(:,NSTAGES+1);
            
          end          
          err0(s,q)=L2Err_hpDG_1D(Coordinates,p,U,qr,err_Shap,U_ex,Tend);
          err1(s,q)=L1Err_hpDG_1D(Coordinates,p,U,qr,err_Shap,U_ex,Tend);
          err2(s,q)=LinfErr_hpDG_1D(Coordinates,p,U,qr,err_Shap,U_ex,Tend);
          h(s,q)=dx;
      end % end of meshsize iteration
  end % end of polynomial degree iteration
  
  s=size(NPOINTS,2);
  fig = figure('Name','Discretization L^2-error,');
  plot(h(:,1),err0(:,1),'r+-',...
       h(:,2),err0(:,2),'b+-',...
       h(:,3),err0(:,3),'g+-',...
       h(:,4),err0(:,4),'r+-',...
       h(:,5),err0(:,5),'b+-',...
       h(:,6),err0(:,6),'g+-')
  grid('on');
  set(gca,'XScale','log','YScale','log','XDir','reverse');
  title('{\bf Discretization errors with respect to L^2 norm}');
  xlabel('{\bf Mesh width}');
  ylabel('{\bf Discretization error}');
  legend('PolyDegree=0','PolyDegree=1','PolyDegree=2','Location','NorthEast');
  p = polyfit(log(h(s-3:s,1)),log(err0(s-3:s,1)),1);
  add_Slope(gca,'NorthEast',p(1));
  p = polyfit(log(h(s-3:s,2)),log(err0(s-3:s,2)),1);
  add_Slope(gca,'East',p(1));
  p = polyfit(log(h(s-3:s,3)),log(err0(s-3:s,3)),1);
  add_Slope(gca,'SouthEast',p(1));
  p = polyfit(log(h(s-3:s,4)),log(err0(s-3:s,4)),1);
  add_Slope(gca,'NorthEast',p(1));
  p = polyfit(log(h(s-3:s,5)),log(err0(s-3:s,5)),1);
  add_Slope(gca,'East',p(1));
  p = polyfit(log(h(s-3:s,6)),log(err0(s-3:s,6)),1);
  add_Slope(gca,'SouthEast',p(1));
  
  fig = figure('Name','Discretization L^1-error,');
  plot(h(:,1),err1(:,1),'r+-',...
       h(:,2),err1(:,2),'b+-',...
       h(:,3),err1(:,3),'g+-',...
       h(:,4),err1(:,4),'r+-',...
       h(:,5),err1(:,5),'b+-',...
       h(:,6),err1(:,6),'g+-')  
  grid('on');
  set(gca,'XScale','log','YScale','log','XDir','reverse');
  title('{\bf Discretization errors with respect to L^1 norm}');
  xlabel('{\bf Mesh width}');
  ylabel('{\bf Discretization error}');
  legend('PolyDegree=0','PolyDegree=1','PolyDegree=2','Location','NorthEast');
  p = polyfit(log(h(s-3:s,1)),log(err1(s-3:s,1)),1);
  add_Slope(gca,'NorthEast',p(1));
  p = polyfit(log(h(s-3:s,2)),log(err1(s-3:s,2)),1);
  add_Slope(gca,'East',p(1));
  p = polyfit(log(h(s-3:s,3)),log(err1(s-3:s,3)),1);
  add_Slope(gca,'SouthEast',p(1));
  p = polyfit(log(h(s-3:s,4)),log(err1(s-3:s,4)),1);
  add_Slope(gca,'NorthEast',p(1));
  p = polyfit(log(h(s-3:s,5)),log(err1(s-3:s,5)),1);
  add_Slope(gca,'East',p(1));
  p = polyfit(log(h(s-3:s,6)),log(err1(s-3:s,6)),1);
  add_Slope(gca,'SouthEast',p(1));
  
  fig = figure('Name','Discretization L^{\infty}-error,');
  plot(h(:,1),err2(:,1),'r+-',...
       h(:,2),err2(:,2),'b+-',...
       h(:,3),err2(:,3),'g+-',...
       h(:,4),err2(:,4),'r+-',...
       h(:,5),err2(:,5),'b+-',...
       h(:,6),err2(:,6),'g+-')
  grid('on');
  set(gca,'XScale','log','YScale','log','XDir','reverse');
  title('{\bf Discretization errors with respect to L^{\infty} norm}');
  xlabel('{\bf Mesh width}');
  ylabel('{\bf Discretization error}');
  legend('PolyDegree=0','PolyDegree=1','PolyDegree=2','Location','NorthEast');
  p = polyfit(log(h(s-3:s,1)),log(err2(s-3:s,1)),1);
  add_Slope(gca,'NorthEast',p(1));
  p = polyfit(log(h(s-3:s,2)),log(err2(s-3:s,2)),1);
  add_Slope(gca,'East',p(1));
  p = polyfit(log(h(s-3:s,3)),log(err2(s-3:s,3)),1);
  add_Slope(gca,'SouthEast',p(1));
  p = polyfit(log(h(s-3:s,4)),log(err2(s-3:s,4)),1);
  add_Slope(gca,'NorthEast',p(1));
  p = polyfit(log(h(s-3:s,5)),log(err2(s-3:s,5)),1);
  add_Slope(gca,'East',p(1));
  p = polyfit(log(h(s-3:s,6)),log(err2(s-3:s,6)),1);
  add_Slope(gca,'SouthEast',p(1));
  clear all;