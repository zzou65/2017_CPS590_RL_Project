...
%                       ((x>mod(0.5+t,X1)).*(x<X1)+(x>0).*(x<mod(1.5+t,X1)));      % Initial data
     U0 = @(x,t)((x>=0).*(x<=1)).*(1-2*abs(x-1/2));                                % Initial data
     U_ex=@(x,t)(((x-t)>=0).*((x-t)<=1)).*(1-2*abs(x-1/2));                        % Initial data
  ...
  print('-depsc', 'rate_L2_LANonSmoothLimit.eps');
  ...
  add_Slope(gca,'SouthEast',p(1));
  print('-depsc', 'rate_L1_LANonSmoothLimit.eps');
  ...
  add_Slope(gca,'SouthEast',p(1));
  print('-depsc', 'rate_Linf_LANonSmoothLimit.eps');
