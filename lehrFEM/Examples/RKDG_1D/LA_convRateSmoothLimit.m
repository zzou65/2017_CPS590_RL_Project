...
% nonlinear SSP Runge Kutta (GottliebShuTadmor2001 SIAM Review)
% no optimal 4th order nonlinear SSP RK
...
% CH-8092 Zurich, Switzerland
  ...
  NPOINTS =4*10.^[1:4];     % Initial set of grid points
  P=[0:2];                  % ploynomial degree 
  CFL=0.9*1./(2*P+1);
  T =0.25;                  % Final time    
  Mmm=0;                   % parameter for modified minmod
%   
...
%       U_ex =@(x,t)1+0.2*sin(pi*(x-t));           % Initial data
      U0 = @(x,t)((x>=0).*(x<=1)).*(1-cos(pi*x).^2);                % Initial data
      U_ex =@(x,t)(((x-t)>=0).*((x-t)<=1)).*(1-cos(pi*(x-t)).^2);           % Initial data
...
%              end
  ...
  print('-depsc', 'rate_L2_LASmoothLimit.eps');
  ...
  print('-depsc', 'rate_L1_LASmoothLimit.eps');
  ...
  print('-depsc', 'rate_Linf_LASmoothLimit.eps');
  clear all;