...
%             plot_QFE(U, Mesh);
%             title(['lambda=' num2str(lambda(j))  ' Interation=' num2str(iter)]);
%             colorbar;
    ...
    title('\bfConvergence rate for L2 error of QFE-DOFs interpolation');
...
%    !gv rate_L2_QFE-DOF.eps &
    ...
    grid on
    set(gca,'XScale','log','YScale','log','XDir','reverse');
    title('\bfConvergence rate for H1 error of QFE-DOFs interpolation');
    xlabel('\bfh','fontsize',14pt);
    ylabel('\bfEnergy error','fontsize',14pt);
    legend(['\bf\lambda=' num2str(lambda(1))],['\bf\lambda=' num2str(lambda(2))],...
           ['\bf\lambda=' num2str(lambda(3))],['\bf\lambda=' num2str(lambda(4))],...
