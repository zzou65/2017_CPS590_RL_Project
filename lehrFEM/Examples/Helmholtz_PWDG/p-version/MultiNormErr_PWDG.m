function [errL2, errH1, errJmp] = MultiNormErr_PWDG(Mesh,u,QuadRule,omega,UHandle,DUHandle,eta,varargin)

% NEW - Andrea
% SQUARE of 3 components of error
% independent of eta
...
%   CH-8092 Zurich, Switzerland
  ...
  for j = 1:nElements
    ...
    if(~isfield(Mesh.ElemData,'Geom') || ~Mesh.ElemData(j).Geom.isParam)
      ...
      errL2 = errL2 + sum(QuadRule.w.*abs(u_EX-u_FE).^2.*det_BK);
      errH1 = errH1 + sum(QuadRule.w.*sum(abs((Du_EX-Du_FE).^2),2).*det_BK);
    ...
    % If an edge of the current element is paramtrized
      ...
      for l=1:size(Mesh0.Elements,1)
        ...
        for k = 1:Mesh.ElemData(j).nDofs
          errL2 = errL2 + sum(QR.w.*abs(u_EX-u_FE).^2.*det_BK);
          errH1 = errH1 + sum(QR.w.*sum(abs((Du_EX-Du_FE).^2),2).*det_BK);
        end
