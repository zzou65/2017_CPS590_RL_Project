...
function main_SemiLagrangianPureCfree(CFL,NREFS)
...
%close all;
clear Mesh;
filename= ['./results/example4_',num2str(CFL)];
%clear all;
...
lambda=0.25;
H1=@(x)exp(-1/(2*lambda^2)*((x(:,1)+0.25).^2+(x(:,2)+0.25).^2)).*(x(:,1)+0.25)/lambda^2;
H2=@(x)exp(-1/(2*lambda^2)*((x(:,1)+0.25).^2+(x(:,2)+0.25).^2)).*(x(:,2)+0.25)/lambda^2;
...
%     plot_Norm_W1F(H_old(:,1),NewMesh);
    ...
    mass(j,6) = L2Err_W1F2nd(NewMesh,H2nd_old,P7O6(),F_HANDLE,0,0);
    mass(j,7) = L2Err_W1F2nd(NewMesh,H2ndb_old,P7O6(),F_HANDLE,0,0);
...
%         plot_Norm_W1F(H2nd_old(1:size(NewMesh.Edges(),1)),NewMesh);
   ...
   % negative velocity
    ...
    L2ErrH(1+nsteps,j,6) = L2Err_W1F2nd(NewMesh,H2nd_old,P7O6(),H_Handle,0,0);
    L2ErrH(1+nsteps,j,7) = L2Err_W1F2nd(NewMesh,H2ndb_old,P7O6(),H_Handle,0,0);
...
%         plot_Norm_W1F(H_old(:,1),NewMesh);
        ...
        nEdges = size(NewMesh.Edges(),1);
        L2nd = assemLoad_W1F2nd(NewMesh,P7O6(),F_HANDLE,i*h);
        H2nd_new = zeros(nEdges,1);
        H2ndb_new = zeros(nEdges,1);
        %FreeDofs = [FreeDofs,nEdges+FreeDofs];
        %H2nd_new(FreeDofs,:) = M2nd(FreeDofs,FreeDofs)\(h*L2nd(FreeDofs,:)+P_pat2nd(FreeDofs,FreeDofs)*H2nd_old(FreeDofs,:));
        H2nd_new = M2nd\(h*L2nd+P_pat2nd*H2nd_old);
        H2ndb_new = M2nd\(h*L2nd+P_qbary2nd*H2ndb_old);
...
%         plot_Norm_W1F(H2nd_old(1:size(NewMesh.Edges(),1)),NewMesh);
    ...
    mass(j,6) = abs(mass(j,6)-L2Err_W1F2nd(NewMesh,H2nd_old,P7O6(),F_HANDLE,0,0))/mass(j,6);
    mass(j,7) = abs(mass(j,7)-L2Err_W1F2nd(NewMesh,H2ndb_old,P7O6(),F_HANDLE,0,0))/mass(j,7);
...
plot(mw,mass(:,1),'rx-',...
    ...
    mw,mass(:,3),'gx-',...
    mw,mass(:,6),'gx--',...
     mw,mass(:,7),'bx--','Linewidth',2,'MarkerSize',8);
grid('on');
set(gca,'YScale','log','XScale','log','FontSize',12,'FontWeight','bold');
xlabel('{\bf mesh width}','FontSize',12,'FontWeight','bold');
...
save([filename,'.mat'],'slope','mw','stopTimeL2ErrH','mass','cfree','CFL','H1','H2','DIR1','DIR2')
save([filename,'.txt']','slope','mw','stopTimeL2ErrH','mass','cfree','CFL','-ASCII')
clear all;
