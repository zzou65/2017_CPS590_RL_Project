...
% model parameters
isigma = 10^-3;             % diffusion constant
mu = 1                  %
...
plot(h,h*(L2err(1,1)/h(1)),'y--',h,L2err(:,1),'r-',h,L2err(:,2),'b-',h); grid('on');
...
plot(h,h*(HCurlSerr(1,1)/h(1)),'y--',h,HCurlSerr(:,1),'r-',h,HCurlSerr(:,2),'b-',h); grid('on');
set(gca,'XScale','log','YScale','log');
xlabel('{\bf h}');
