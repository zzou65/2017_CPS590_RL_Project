function main_AdjSemiLagrangianSchemes(CFL,NREFS)
...
clear Mesh;
filename= ['./results/exampleAdj1_',num2str(CFL)];
%clear all;
...
for j=1:NREFS
    ...
    for i = 1:nsteps
        ...
        %righthand side
        Lf = assemCochain_1f(NewMesh, G_HANDLE,gauleg(0,1,10),i*h);
        L=[Lf Lf];
        L = h*M*L+[Mm*P_i *H_old(:,1) P_q'*H_old(:,2)]-A*H_new;
   ...
   plot_Norm_W1F(H_new(:,1),NewMesh);colorbar;
   plot_Norm_W1F(H_new(:,2),NewMesh);colorbar;
...
plot(mw,stopTimeL2ErrH(:,1),'rx-',...
    mw,stopTimeL2ErrH(:,2),'bx-','Linewidth',2,'MarkerSize',8);
grid('on');
...
legend('Interpolation','Patch','Location','Northwest');
