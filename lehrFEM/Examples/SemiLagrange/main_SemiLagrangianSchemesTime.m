function main_SemiLagrangianSchemesTime(CFL,NREFS_init)
...
clear Mesh;
filename= ['./results/example1Time_',num2str(CFL)];
%clear all;
%NREFS_init = 4;     % Number of uniform red refinements
NREFS = length(CFL);
JIG =1;
...
for j=1:NREFS
    ...
    h(j) = CFL(j)*mw/norm([v1,v2]);
    nsteps = ceil((T1-T0)/h(j));
    h(j) = h(j)-(T0+nsteps*h(j)-T1)/nsteps;
    steps(j) = nsteps;
    ...
    % Quadrature
    pbBm = trace_bcenters(NewMesh, Dir_Handle,JvDir_Handle,-h(j));
    ...
    for i = 1:nsteps
        [i nsteps]
        time(i+1,j) = h(j)+time(i,j);

        %Dirichlet data
        [H_Dir,FreeDofs] = assemDir_W1F(NewMesh,-1,H_Handle,gauleg(0,1,5),(i)*h(j));
        ...
        %righthand side
        Lf = assemCochain_1f(NewMesh, F_HANDLE,gauleg(0,1,10),i*h(j));
        L=[Lf Lf Lf Lf Lf];
        L = h(j)*M*L+[M*P_i*H_old(:,1) P_qbary*H_old(:,2) P_q*H_old(:,3) (M-Lie_p)*H_old(:,4) (M-Lie_i)*H_old(:,5)]-A*H_new;
        ...
        L2ErrH(i+1,j,1) = L2Err_W1F(NewMesh,H_old(:,1),P7O6(),H_Handle,0,i*h(j));
        L2ErrH(i+1,j,2) = L2Err_W1F(NewMesh,H_old(:,2),P7O6(),H_Handle,0,i*h(j));
        L2ErrH(i+1,j,3) = L2Err_W1F(NewMesh,H_old(:,3),P7O6(),H_Handle,0,i*h(j));
        L2ErrH(i+1,j,4) = L2Err_W1F(NewMesh,H_old(:,4),P7O6(),H_Handle,0,i*h(j));
        L2ErrH(i+1,j,5) = L2Err_W1F(NewMesh,H_old(:,5),P7O6(),H_Handle,0,i*h(j));
       
        % 2nd
        nEdges = size(NewMesh.Edges(),1);
        L2nd = assemLoad_W1F2nd(NewMesh,P7O6(),F_HANDLE,i*h(j));
        H2nd_new = zeros(2*nEdges,1);
        H2ndb_new = zeros(2*nEdges,1);
        H2nd_new = M2nd\(h(j)*L2nd+P_pat2nd*H2nd_old);
        H2ndb_new = M2nd\(h(j)*L2nd+P_qbary2nd*H2ndb_old);
        
        %Update vectors
        H2nd_old = H2nd_new;
        H2ndb_old = H2ndb_new;
        
        L2ErrH(i+1,j,6) = L2Err_W1F2nd(NewMesh,H2nd_old,P7O6(),H_Handle,0,i*h(j));
        L2ErrH(i+1,j,7) = L2Err_W1F2nd(NewMesh,H2ndb_old,P7O6(),H_Handle,0,i*h(j));
...
figure;
hold on;
plot(h,stopTimeL2ErrH(:,1),'rx-',...
    h,stopTimeL2ErrH(:,2),'bx-',...
    h,stopTimeL2ErrH(:,3),'gx-',...
    h,stopTimeL2ErrH(:,6),'gx--',...
    h,stopTimeL2ErrH(:,7),'bx--','Linewidth',2,'MarkerSize',8);
grid('on');
set(gca,'YScale','log','XScale','log','FontSize',12,'FontWeight','bold');
xlabel('{\bf timestep}','FontSize',12,'FontWeight','bold');
...
slope=[diff(log(stopTimeL2ErrH(:,1)))./diff(log(h)) ...
    diff(log(stopTimeL2ErrH(:,2)))./diff(log(h)) ...
    diff(log(stopTimeL2ErrH(:,3)))./diff(log(h)) ...
    diff(log(stopTimeL2ErrH(:,4)))./diff(log(h)) ...
    diff(log(stopTimeL2ErrH(:,5)))./diff(log(h)) ...
    diff(log(stopTimeL2ErrH(:,6)))./diff(log(h))...
    diff(log(stopTimeL2ErrH(:,7)))./diff(log(h))]
