function varargout= assemMat_SemiLagQuad_W1F_patchesCheap(Mesh, defMesh,intersec,varargin)
% 
...
for i = 1:nElements
  ...
  for j = 1:intersec(i).nElems
      if (polygons(j).Elem==[Mesh.Neigh(i),i])
          Aloc=zeros(3,3);
...
if(nargout > 1)
    ...
    varargout{1} = sparse(I(1:loc-9),J(1:loc-9),A(1:loc-9,1:loc-9),nEdges,nEdges);
end
