function driver_ad_error_exp1(CFL)
...
H1 = @(x)  (1-x(:,1).^2).^2 .*(x(:,2)-x(:,2).^3);
D1H1=@(x) -4 * (x(:,1)-x(:,1).^3) .*(x(:,2)-x(:,2).^3);
D2H1=@(x) (1-x(:,1).^2).^2 .*(1-3*x(:,2).^2);
...
filename=['./figures/L2ErrAdjoint1',sprintf('%1.2f',CFL),'.mat'];
...
filename=['./figures/L2ErrAdjoint1',sprintf('%1.2f',CFL),'.fig'];
saveas(gcf,filename);
filename=['./figures/L2ErrAdjoint1',sprintf('%1.2f',CFL),'.eps'];
