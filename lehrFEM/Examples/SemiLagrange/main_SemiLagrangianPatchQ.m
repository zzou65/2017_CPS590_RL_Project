...
function main_SemiLagrangianPatchQ(CFL,NREFS)
...
for j=1:NREFS
    ...
    P_pat2nd1 = assemMat_SemiLagQuad_W1F_patches2ndQuad(NewMesh, defMesh,intersec,P1O2());
    P_pat2nd2 = assemMat_SemiLagQuad_W1F_patches2ndQuad(NewMesh, defMesh,intersec,P3O3());
    P_pat2nd3 = assemMat_SemiLagQuad_W1F_patches2ndQuad(NewMesh, defMesh,intersec,P7O6());
        
    H2ndb_old1 = M2nd\assemLoad_W1F2nd(Mesh,P7O6(),H_Handle,0);
    H2ndb_old2 = M2nd\assemLoad_W1F2nd(Mesh,P7O6(),H_Handle,0);
    H2ndb_old3 = M2nd\assemLoad_W1F2nd(Mesh,P7O6(),H_Handle,0);
        
    time(1,j) = 0;
    L2ErrH(1,j,1) = L2Err_W1F2nd(NewMesh,H2ndb_old1,P7O6(),H_Handle,0,0);
    L2ErrH(1,j,2) = L2Err_W1F2nd(NewMesh,H2ndb_old2,P7O6(),H_Handle,0,0);
    L2ErrH(1,j,3) = L2Err_W1F2nd(NewMesh,H2ndb_old3,P7O6(),H_Handle,0,0);
    ...
    for i = 1:nsteps
        ...
        H2ndb_new1 = M2nd\(h*L2nd+P_pat2nd1*H2ndb_old1);
        H2ndb_new2 = M2nd\(h*L2nd+P_pat2nd2*H2ndb_old2);
        H2ndb_new3 = M2nd\(h*L2nd+P_pat2nd3*H2ndb_old3);
        
        %Update vectors
        H2ndb_old1 = H2ndb_new1;
        H2ndb_old2 = H2ndb_new2;
        H2ndb_old3 = H2ndb_new3;
        
        L2ErrH(i+1,j,1) = L2Err_W1F2nd(NewMesh,H2ndb_old1,P7O6(),H_Handle,0,i*h);
        L2ErrH(i+1,j,2) = L2Err_W1F2nd(NewMesh,H2ndb_old2,P7O6(),H_Handle,0,i*h);
        L2ErrH(i+1,j,3) = L2Err_W1F2nd(NewMesh,H2ndb_old3,P7O6(),H_Handle,0,i*h);
...
plot(mw,stopTimeL2ErrH(:,1),'rx-',...
    ...
    mw,stopTimeL2ErrH(:,3),'gx-','Linewidth',2,'MarkerSize',8);
grid('on');
...
legend('P103','P3O3','P7O6','Location','Northwest');
