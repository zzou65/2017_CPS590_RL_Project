function varargout = assemMat_W1F2nd(Mesh,EHandle,varargin)
% ASSEMMAT_W1F Assembly for *edge elements* in 2D
%
%   A = ASSEMMAT_W1F(MESH,EHANDLE) assembles the global matrix from the
%   local element contributions given by the function handle EHANDLE and
%   returns the matrix in a sparse representation.
%
%   A = ASSEMMAT_W1F(MESH,EHANDLE,EPARAM) handles the variable length
...
%   EHandle = @STIMA_Curl_W1F;
%   A = assemMat_W1F(Mesh,EHandle);
%  
...
%   CH-8092 Zurich, Switzerland
  ...
  for i = 1:nElements
    ...
    Peori = diag([p1 p2 p3 1 1 1]); % scaling matrix taking into account orientations
