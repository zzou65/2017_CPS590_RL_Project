function main_EulerianSchemes(CFL,NREFS)
...
for j=1:NREFS
    ...
    %Eulerian
    % patches
    Jump = assemMat_Inn_LieW1F(NewMesh,@STIMA_Inn_LiePennW1F,Dir_Handle,gauleg(0,1,5),0.5);
    ...
    L2ErrH(1,j,3) = L2Err_W1F(NewMesh,H_exL2_old,P7O6(),H_Handle,0,0);
    L2ErrH(1,j,4) = L2Err_W1F(NewMesh,H_exIP_old,P7O6(),H_Handle,0,0);
    
    JErrH(1,j,1) = H_old(:,1)'*Jump*H_old(:,1);
    JErrH(1,j,2) = H_old(:,2)'*Jump*H_old(:,2);
    JErrH(1,j,3) = H_exL2_old'*Jump*H_exL2_old;
    JErrH(1,j,4) = H_exIP_old'*Jump*H_exIP_old;
    ...
    for i = 1:nsteps
        ...
        L=[Lf Lf];
        L = h*M*L+[(M-h*Lie_p)*H_old(:,1) (M-h*Lie_i)*H_old(:,2)]-A*H_new;
        ...
        JErrH(i+1,j,1) = H_old(:,1)'*Jump*H_old(:,1);
        JErrH(i+1,j,2) = H_old(:,2)'*Jump*H_old(:,2);
        ...
        L2ErrH(i+1,j,3) = L2Err_W1F(NewMesh,H_exL2_new,P7O6(),H_Handle,0,i*h);
        JErrH(i+1,j,3) = H_exL2_old'*Jump*H_exL2_old;
        ...
        Lf = assemCochain_1f(NewMesh, F_HANDLE,gauleg(0,1,10),i*h);
        A= (M+h*Lie_i);
        L = h*M*Lf+M*H_exIP_old;
        H_exIP_new = A\L;
        L2ErrH(i+1,j,4) = L2Err_W1F(NewMesh,H_exIP_new,P7O6(),H_Handle,0,i*h);
        JErrH(i+1,j,4) = H_exIP_old'*Jump*H_exIP_old;
        H_exIP_old = H_exIP_new;
        
%         plot_Norm_W1F(H_exL2_new,NewMesh); colorbar;
...
for j=1:NREFS
   ...
   stopTimeJErrH(j,:)=sqrt(JErrH(steps(j)+1,j,:))
end
...
slope=[diff(log(stopTimeJErrH(:,1)))./diff(log(mw)) ...
    diff(log(stopTimeJErrH(:,2)))./diff(log(mw)) ...
    diff(log(stopTimeJErrH(:,3)))./diff(log(mw)) ...
    diff(log(stopTimeJErrH(:,4)))./diff(log(mw))]
...
plot(mw,stopTimeL2ErrH(:,1),'rx-',...
    ...
    mw,stopTimeL2ErrH(:,3),'rx--',...
    mw,stopTimeL2ErrH(:,4),'bx--','Linewidth',2,'MarkerSize',8);
grid('on');
...
figure;
plot(mw,stopTimeJErrH(:,1),'rx-',...
    mw,stopTimeJErrH(:,2),'bx-',...
    mw,stopTimeJErrH(:,3),'rx--',...
    mw,stopTimeJErrH(:,4),'bx--','Linewidth',2,'MarkerSize',8);
grid('on');
set(gca,'YScale','log','XScale','log','FontSize',12,'FontWeight','bold');
xlabel('{\bf mesh width}','FontSize',12,'FontWeight','bold');
ylabel('{\bf Jump-error}','FontSize',12,'FontWeight','bold');
legend('Patch explicit','Interpolation explicit','Patch implicit','Interpolation implicit','Location','Northwest');
