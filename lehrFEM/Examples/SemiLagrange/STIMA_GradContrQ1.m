function Mloc = STIMA_GradContrQ1(Vertices,ElemInfo,V_HANDLE,varargin)
% STIMA_GradContr DG-like fluxes
%
...
%   
  ...
  % upwind edges
  theta=ones(6,1);
%   if (1/2*sum(Fval1,1)*N1' < 0)
%       theta(1:2)=0;
%   end
%   if (1/2*sum(Fval2,1)*N2' < 0)
%       theta(3:4)=0;
%   end
%   if (1/2*sum(Fval3,1)*N3' < 0)
%       theta(4:6)=0;
%   end
  ...
  b1n=theta.*sum(b1.*([ones(nPoints,1)*N1;...
                        ones(nPoints,1)*N2;...
                        ones(nPoints,1)*N3]),2);
 
  b2n=theta.*sum(b2.*([ones(nPoints,1)*N1;...
                        ones(nPoints,1)*N2;...
                        ones(nPoints,1)*N3]),2);
 
  b3n=theta.*sum(b3.*([ones(nPoints,1)*N1;...
                        ones(nPoints,1)*N2;...
                        ones(nPoints,1)*N3]),2);
