...
T_Handle=@(t,varargin) cos(2*pi*t);%
DT_Handle=@(t,varargin) -2*pi*sin(2*pi*t);%
...
for j=1:NREFS
    ...
    %pbV2 = trace_vertices_W(NewMesh,Dir_Handle,JvDir_Handle,h);
    %P = assemMat_SemiLagQuad_W1F(NewMesh, pbV2);                                      % vertex
    pbB = trace_bcenters(NewMesh, Dir_Handle,JvDir_Handle,h);
    P = assemMat_SemiLagQuad_W1F_bary(NewMesh, pbB);                                   % barycenter
    %P = assemMat_SemiLagQuad_W1F_strang1(NewMesh, @(x)Dir_Handle(x),JvDir_Handle,h);    % strang1
