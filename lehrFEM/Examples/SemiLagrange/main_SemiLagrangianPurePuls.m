...
function main_SemiLagrangianPurePuls(CFL,NREFS)
...
R=0.3;
C=[-0.5,-0.5];
H1=@(x)Const_pulse_2D(x,R,C);
H2=@(x)Const_pulse_2D(x,R,C);
...
%         plot_Norm_W1F(H2nd_old(1:size(NewMesh.Edges(),1)),NewMesh);
    ...
    mass(j,4) = L2Norm_W1FOutCircle(NewMesh,H_old(:,4),P7O6(),R,C);
    mass(j,5) = L2Norm_W1FOutCircle(NewMesh,H_old(:,5),P7O6(),R,C);
   mass(j,6) = L2Norm_W1F2ndOutCircle(NewMesh,H2nd_old,P7O6(),R,C);
   mass(j,7) = L2Norm_W1F2ndOutCircle(NewMesh,H2ndb_old,P7O6(),R,C);

    figure;plot_W1F(H_old(:,1),NewMesh);
    figure;plot_W1F(H_ref,NewMesh);
end%  Mesh refinement
