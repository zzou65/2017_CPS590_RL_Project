function main_SemiLagrangianSchemesJig(CFL,NREFS)
...
clear Mesh;
filename= ['./results/example1Jig_',num2str(CFL)];
%clear all;
...
for j=1:NREFS
    ...
    L2ErrH(1,j,3) = L2Err_W1F2nd(NewMesh,H2nd_old,P7O6(),H_Handle,0,0);
    ...
    for i = 1:nsteps
        ...
        L=[Lf Lf];
        L = h*M*L+[M*P_i*H_old(:,1)  P_q*H_old(:,2)]-A*H_new;
        ...
        L2ErrH(i+1,j,3) = L2Err_W1F2nd(NewMesh,H2nd_old,P7O6(),H_Handle,0,i*h);
...
legend('Interpolation','Patch','Patch2nd','Location','Northwest');
