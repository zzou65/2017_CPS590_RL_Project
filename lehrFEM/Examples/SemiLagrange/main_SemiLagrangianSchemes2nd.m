function main_SemiLagrangianSchemes2nd(CFL,NREFS)
...
clear Mesh;
filename= ['./results/example12nd_',num2str(CFL)];
%clear all;
...
legend('Patch2nd','Quadrature2nd','Location','Northwest');
saveas(gcf,[filename,'error.fig'])
print('-depsc' ,[filename,'error.eps'])

slope=[diff(log(stopTimeL2ErrH(:,6)))./diff(log(mw))...
