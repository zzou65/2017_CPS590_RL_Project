function main_SemiLagrangianQuadQuadmod(CFL,NREFS)
...
clear Mesh;
filename= ['./results/example5mod_',num2str(CFL)];
%clear all;
...
for j=1:NREFS
    ...
    L2ErrH(1,j,3) = L2Err_W1F(NewMesh,Hm_old(:,1),P7O6(),H_Handle,0,0);
    L2ErrH(1,j,4) = L2Err_W1F(NewMesh,Hm_old(:,2),P7O6(),H_Handle,0,0);
    ...
    for i = 1:nsteps
        ...
        L=[Lf Lf];
        L = h*M*L+[P_qbaryB*H_old(:,2) P_qbaryS*H_old(:,2)]-A*H_new;
        ...
        Lfm = assemCochain_1f(NewMesh, F_mod_HANDLE,gauleg(0,1,10),i*h);
        Lm=[Lfm Lfm];
        Lm = h*M*Lm+[P_qbaryB*Hm_old(:,2) P_qbaryS*Hm_old(:,2)]-A*Hm_new;
       
        %solve system
        Hm_new(FreeDofs,:) = A(FreeDofs,FreeDofs)\Lm(FreeDofs,:);
       
        %Update vectors
        Hm_old=Hm_new;
       
        L2ErrH(i+1,j,3) = L2Err_W1F(NewMesh,Hm_old(:,1),P7O6(),H_Handle,0,i*h);
        L2ErrH(i+1,j,4) = L2Err_W1F(NewMesh,Hm_old(:,2),P7O6(),H_Handle,0,i*h);
...
legend('1st low','1st high','1st lowMod','1st highMod','h','Location','Northwest');
