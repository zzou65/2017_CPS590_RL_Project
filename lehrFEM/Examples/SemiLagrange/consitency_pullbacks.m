...
%    pulled_back_bcenters = trace_bcenters(NewMesh,@(x)-dt*Dir_HANDLE(x));
    A1 = assemMat_SemiLag_W1F(NewMesh, pulled_back_vertices);
    B1 = assemMat_SemiLagQuad_W1F(NewMesh, pulled_back_vertices,JDir_Handle,dt);
%    B1b = assemMat_SemiLagQuad_W1F_bary(NewMesh, pulled_back_bcenters,JDir_Handle,dt);
    B1s = assemMat_SemiLagQuad_W1F_strang1(NewMesh, @(x)-dt*Dir_HANDLE(x),JDir_Handle,dt);
 ...
 %   pulled_back_bcenters = trace_bcenters(NewMesh,@(x)dt*Dir_HANDLE(x));
  ...
  %  B2b = assemMat_SemiLagQuad_W1F_bary(NewMesh, pulled_back_bcenters,JDir_Handle,-dt);
    B2s = assemMat_SemiLagQuad_W1F_strang1(NewMesh, @(x)dt*Dir_HANDLE(x),JDir_Handle,-dt);
...
%     Err(i,1) = abs(((M\B1b)*u-pb_c)'*M*((M\B1b)*u-pb_c));
%     Err(i,2) = abs(((M\B1s)*u-pb_c)'*M*((M\B1s)*u-pb_c));
%     Err(i,3) = abs((A1*u-pb_c)'*M*(A1*u-pb_c));
%     Err(i,4) = abs(((M\B1)*u-pb_c)'*M*((M\B1)*u-pb_c));
%     Err(i,5) = abs(((M\B2b')*u-pb_ad_c)'*M*((M\B2b')*u-pb_ad_c));
%     Err(i,6) = abs(((M\B2s')*u-pb_ad_c)'*M*((M\B2s')*u-pb_ad_c));
%     Err(i,7) = abs(((M\A2'*M)*u-pb_ad_c)'*M*((M\A2'*M)*u-pb_ad_c));
%     Err(i,8) = abs(((M\B2')*u-pb_ad_c)'*M*((M\B2')*u-pb_ad_c));
    
%     Err(i,1) = norm(B1b*u-M*pb_c);
%     Err(i,2) = norm(B1s*u-M*pb_c);
%     Err(i,3) = norm(A1*u-pb_c);
%     Err(i,4) = norm(B1*u-M*pb_c);
%     Err(i,5) = norm(B2b'*u-M*pb_ad_c);
%     Err(i,6) = norm(B2s'*u-M*pb_ad_c);
%     Err(i,7) = norm(A2'*M*u-M*pb_ad_c);
%     Err(i,8) = norm(B2'*u-M*pb_ad_c);
