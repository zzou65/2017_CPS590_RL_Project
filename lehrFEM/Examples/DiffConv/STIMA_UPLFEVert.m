function UP_loc = STIMA_UPLFEVert(Vertices,vHandle, mass, varargin)
...
%   CH-8092 Zurich, Switzerland
  ...
  yhat = (a1+v(1,:)-bK)*inv_BK;
  if(yhat(2) >= 0 && yhat(1) >= 0)
      elem= mass(1)*[-gradN(1,[1 2])*v(1,:)',...
                 -gradN(1,[3 4])*v(1,:)',...
                 -gradN(1,[5 6])*v(1,:)'];
    UP_loc(1,:) = elem; 
  end
  if (yhat(2)==0 || yhat(1)==0)
     UP_loc(1,:)=1/2* UP_loc(1,:);%two elements contribute to matrix (what if edge on the boundary??????)
  end
  
  % Compute Lie derivative scheme for second vertex using upwind quadrature    
  yhat = (a2+v(2,:)-bK)*inv_BK;
  if(yhat(1) <= 0 && yhat(2) >= 0 && sum(yhat) <= 0)
    elem = mass(2)*[-gradN(2,[1 2])*v(2,:)', ...
                -gradN(2,[3 4])*v(2,:)', ...
                -gradN(2,[5 6])*v(2,:)'];
    UP_loc(2,:) = elem;
  end
  if (yhat(2) == 0 || sum(yhat) == 0)
     UP_loc(2,:)=1/2* UP_loc(2,:);
  end

  % Compute Lie derivative scheme for third midpoint using upwind quadrature
  yhat = (a3+v(3,:)-bK)*inv_BK;
  if(yhat(2) <= 0 && yhat(1) >= 0 && sum(yhat) <= 0)
    elem = mass(3)*[-gradN(3,[1 2])*v(3,:)', ... 
                -gradN(3,[3 4])*v(3,:)', ...
                -gradN(3,[5 6])*v(3,:)'];
    UP_loc(3,:) = elem;
  end
