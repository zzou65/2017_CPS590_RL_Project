...
%   CH-8092 Zurich, Switzerland 
    ...
    for i=1:6
        ...
        figure
        plot([0 1 0 0],[0 0 1 0],'.-k','linewidth',2,'markersize',10)
        hold on
        loc = find(sum([X Y].^2,2) < TOL | sum([X-1 Y].^2,2) < TOL | sum([X Y-1].^2,2) < TOL);
        [pos dummy1 dummy2] = find(shap(loc,i));
        loc = loc(pos);
        plot3([X(loc) X(loc)] ,[Y(loc) Y(loc)],[zeros(size(loc)) shap(loc,i)],'--k','linewidth',2)
        plot_Shap([X Y],shap(:,i));
        
        % Output .eps files

        FileName = ['Shap_QFE3D_' int2str(i) '.eps'];
