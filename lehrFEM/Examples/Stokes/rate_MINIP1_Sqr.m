% Run script for the convergence rate for MINI stokes solver on the unit 
% square domain.
...
%   CH-8092 Zurich, Switzerland
    ...
    for i=1:NREFS
        ...
        L2_U1 = L2Err_MINI(Mesh,U_V1,P7O6(),UEX_Handle1);
        L2_U2 = L2Err_MINI(Mesh,U_V2,P7O6(),UEX_Handle2);
        Err_L2_U(i) = sqrt(L2_U1^2+L2_U2^2);
        HCS_U1 = H1SErr_MINI(Mesh,U_V1,P7O6(),UEX_GRAD_Handle1); 
        HCS_U2 = H1SErr_MINI(Mesh,U_V2,P7O6(),UEX_GRAD_Handle2); 
        Err_HCS_U(i) = sqrt(HCS_U1^2+HCS_U2^2);
        Err_L2_P(i) = L2Err_LFE(Mesh,U_P,P7O6(),UEX_Handle_P);
        save rate_MINIP1_NC_Sqr.mat M_W Err_L2_U Err_HCS_U Err_L2_P
    ...
    title('\bfConvergence rate of discretization error for MINI elements')
    legend('\bf\fontsize{10}V, L2 Norm','\bf\fontsize{10}V, H1 Semi-norm',...
           '\bf\fontsize{10}P, L2 Norm');
    
    % Generate .eps files
    
    print('-depsc', 'rate_MINIP1_NC_Sqr.eps');
