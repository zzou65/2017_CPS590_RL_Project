% Run script for the convergence rate for P2P0 stokes solver on the unit 
% square domain.
...
%   CH-8092 Zurich, Switzerland
    ...
    for i=1:NREFS
        ...
        % Generate non-complete REG-refined mesh
        ...
        [U,FreeDofs] = assemDir_Stokes_P2P0(Mesh,-1,GD_Handle);
        L = L - A*U;
        ...
        U_V1 = U(1:offset);
        U_V2 = U((offset+1):(2*offset));
        U_P = U((2*offset+1):(2*offset+nElements));
        
        % Compute discretized error
        
        L2_U1 = L2Err_QFE(Mesh,U_V1,P7O6(),UEX_Handle1);
        L2_U2 = L2Err_QFE(Mesh,U_V2,P7O6(),UEX_Handle2);
        Err_L2_U(i) = sqrt(L2_U1^2+L2_U2^2);
        HCS_U1 = H1SErr_QFE(Mesh,U_V1,P7O6(),UEX_GRAD_Handle1); 
        HCS_U2 = H1SErr_QFE(Mesh,U_V2,P7O6(),UEX_GRAD_Handle2); 
        Err_HCS_U(i) = sqrt(HCS_U1^2+HCS_U2^2);
        Err_L2_P(i) = L2Err_P0(Mesh,U_P,P7O6(),UEX_Handle_P);
        save rate_P2P0_NC_Sqr.mat M_W Err_L2_U Err_HCS_U Err_L2_P
