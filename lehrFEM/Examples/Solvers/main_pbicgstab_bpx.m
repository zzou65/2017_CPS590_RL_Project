% Run script for preconditioned BiCG stabilized solver.
...
%   CH-8092 Zurich, Switzerland
  ...
  t = cputime;
  U0 = L;
  [U(CFDofs),flag,relres,iter,resvec] = pbicgstab_solve(U0,A,L,TOL,MAXIT, ...
                                                        @bpx_prec,ML_Data);
  fprintf('Runtime of preconditioned BiCG stabilized solver [s]  :  %f\n',cputime-t);
     
  fig = figure('Name','Preconditioned BiCG stabilized solver');
  plot(resvec,'rx');
  title('{\bf Preconditioned BiCG stabilized solver (BPX preconditioner)}');
