...
% CH-8092 Zurich, Switzerland
  ...
  t = cputime;
  U0 = zeros(size(L));
  [U(FreeDofs),flag,relres,iter,resvec] = bicgstab_solve(U0,A,L,TOL,MAXIT);
  fprintf('Runtime of BiCG stabilized solver [s]  :  %f\n',cputime-t);
  
  fig = figure('Name','BiCG stabilized solver');
  plot(resvec,'rx');
  title('{\bf BiCG stabilized solver}');
