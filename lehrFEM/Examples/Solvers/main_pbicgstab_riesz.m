% Run script for BiCG stabilized solver with Riesz preconditioner.
...
%   CH-8092 Zurich, Switzerland
  ...
  [U(mg_data{end}.dofs),flag,relres,iter,resvec] = ...
    pbicgstab_solve(U0,A,L,TOL,MAXIT,@mg,IT,mg_data);
  fprintf('Runtime of preconditioned BiCG stabilized solver [s]  :  %f\n',cputime-t);
  
  fig = figure('Name','Preconditioned BiCG stabilized solver');
  plot(resvec,'rx');
  title('{\bf Preconditioned BiCG stabilized solver (operator preconditioner)}');  
