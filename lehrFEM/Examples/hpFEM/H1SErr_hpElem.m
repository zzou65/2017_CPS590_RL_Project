function err = H1SErr_hpElem(Mesh,u,Elem2Dof,QuadRule,Shap,FHandle,varargin)
% H1SERR_HP elementwise discretization error in H1 semi-norm for hp finite elements.
%
%   ERR = H1SERR_HP(MESH,U,ELEM2DOF,QUADRULE,SHAP,FHANDLE) computes the
...
%   struct MESH elementwise, err is a vector with nElements entries.
%
...
%   QUADRULE.
%
%   ERR = H1SERR_HP(MESH,U,ELEM2DOF,QUADRULE,SHAP,FHANDLE,FPARAM) also
%   handles the variable length argument list FPARAM to the exact solution
%   FHANDLE.
%
%   Example:
% 
%   err = H1SErr_hp(Mesh,u,Elem2Dof,QuadRule,Shap,Fhandle);
