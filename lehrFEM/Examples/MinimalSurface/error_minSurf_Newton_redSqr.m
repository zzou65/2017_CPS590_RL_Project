% error_minSurf_Newton_redSqr
%   Error analysis of the minimal surface problem using Newtons method on a
%   uniform square mesh with corner coordinates 
%   (-1/2,-1/2),(-1/2,1/2),(1/2,1/2),(-1/2,1/2)
%   Exact solution is given by U = sqrt(cosh(y).^2 - x.^2). The gradient
%   of U is continious in the domain
%   The plot of the discretization error against mesh width is saved as 
%   error_Newton_redSqr.eps and the error plot against degrees of freedom
%   is saved as error_Newton_redSqr_DF.eps
...
U0 = @(x,varargin)U_exact(x,varargin{:}) + (1/4 - x(:,1).^2) .* (1/4 - x(:,2).^2);  % Initial guess
U_grad = @Ugrad;                                                                    % Gradient of solution
...
Mesh = load_Mesh('Coord_Sqr3.dat','Elem_Sqr2.dat');
...
print -depsc error_Newton_redSqr.eps
...
print -depsc error_Newton_redSqr_DF.eps