% error_minSurf_Newton_Circ
%   Error analysis of the minimal surface problem using Newtons method on a
%   uniform circular mesh with center at the origin and radius 1. The plot
%   of the error against mesh width is saved as error_Newton_Circ.eps and
%   the plot of the error against the degrees of freedom is saved as
%   error_Newton_Circ_DF.eps
...
U0 = @(x,varargin)U_exact(x,varargin{:}) + 5*(1 - x(:,1).^2 - x(:,2).^2);         % Initial guess
U_grad = @Ugrad;
...
print -depsc error_Newton_Circ.eps
...
print -depsc error_Newton_Circ_DF.eps