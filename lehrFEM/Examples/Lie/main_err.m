...
%   CH-8092 Zurich, Switzerland
    ...
    e_m=0;
    e_c=1;
    d=getData(5);                   % struct containing Coordinates, Elements and various function handles
...
%        M_P0=assemMAT_P0(Mesh,@MASS_P0);
        ...
        c1Bd=assemBndCochain_1f(New_Mesh,[-1],d.SOL1_Handle,gauleg(0,1,10)); % boundary 
...
%        Lf=M_W1F*cf-S*cU;
       ...
       % solve equation
       %cU(FreeDofs)=S(FreeDofs,FreeDofs)\Lf(FreeDofs);
       cU_st(FreeDofs)=S_st(FreeDofs,FreeDofs)\Lf_st(FreeDofs);
       cU_up(FreeDofs)=S_up(FreeDofs,FreeDofs)\Lf_up(FreeDofs);

       plot_Norm_W1F(cU_st,New_Mesh); colorbar;
       plot_Norm_W1F(cU_up,New_Mesh); colorbar;
%        err1(i)=L2Err_W1F(New_Mesh,cU,QuadRule,d.U_EX_Handle)
%        err2(i)=HCurlSErr_W1F(New_Mesh,cU,QuadRule,d.CURL_U_EX_Handle)
