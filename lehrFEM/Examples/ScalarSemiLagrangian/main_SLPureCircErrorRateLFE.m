function main_SLPureCircErrorRateLFE(NREFS, CFLs, locIntegrate, Problem,timestepchoice)
...
for j=1:NREFS
    ...
    if j >1  
        ...
        %Loc = unique([NewMesh.Edges(Loc,1);New Mesh.Edges(Loc,2)]);
    ...
    for k = 1:size(CFLs,2)
        ...
        % Mex = assemMat_LFE(NewMesh,@MASS_LFE);
...
sdata = size(data,1);
fid = fopen(['./results/LFE_RotHillscalarL2rate_',locIntegrate,'_',Problem,'_',num2str(CFLs(1)),'_',num2str(timestepchoice),'.txt'], 'wt');
