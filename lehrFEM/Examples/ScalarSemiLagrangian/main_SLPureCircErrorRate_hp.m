...
for j=1:NREFS
    ...
    for k = 1:size(CFLs,2)
        ...
        [P_p,M] = assemMat_SL_hp(NewMesh, defMesh, Elem2Dof,intersec,quadflag,QuadRule_2D, polydegree);
        ...
        %u_old = M\assemLoad_hp(NewMesh,Elem2Dof,QuadRule_2D,Shap_2D,u_Handle);
        u_old = M\assemLoad_hp_SL(NewMesh,defMesh,Elem2Dof,intersec, quadflag,QuadRule_2D, polydegree,u_Handle);
...
sdata = size(data,1);
fid = fopen(['./results/hp_RotHillscalarL2rate_',locIntegrate,'_',Problem,'_',num2str(CFLs(1)),'_',num2str(polydegree),'.txt'], 'wt');
