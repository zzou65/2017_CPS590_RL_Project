function main_SLPureCircErrorTestLFE(NREFS, CFLs, locIntegrate, Problem,timestepchoice)
...
% parameters
NREFS_init = 5;                            % Number of initial uniform red refinements
...
for j=1:NREFS
    ...
    for k = 1:size(CFLs,2)
        ...
        P_p= assemMat_SLpullback_LFE(NewMesh, defMesh,intersec,quadflag,QuadRule);
                
        % Assemble MASS matrix
        %M = assemMat_LFE(NewMesh,@MASS_LFE);
        %M1 = assemMat_SLmass_LFEquad(NewMesh, defMesh,intersec);
        M = assemMat_SLmass_LFE(NewMesh, defMesh,intersec,quadflag,QuadRule);
        %RM = chol(M); 
       
        % inital data
        u_old = M\assemLoad_LFE_SL(NewMesh,defMesh,intersec, quadflag,P7O6(),u_Handle);
        plot_LFE(u_old,NewMesh);
        colorbar; CLim1=get(gca,'CLim');
        
%         u_old = M\assemLoad_LFE(NewMesh,P7O6(),u_Handle);
%         plot_LFE(u_old,NewMesh);
%         colorbar; CLim1=get(gca,'CLim');
        ...
        for i = 1:steps(j,k)
            ...
            u_end = M\assemLoad_LFE(NewMesh,P7O6(),@(x,d)sol_Handle(x,time(i+1)));
            plot_LFE(u_end-u_old,NewMesh); colorbar
        ...
        %rectangle('Position',[-0.5,-0.5,1,1],'Curvature',[1,1])
        set(gca,'CLim',CLim1); colorbar;
        u_end = M\assemLoad_LFE(NewMesh,P7O6(),@(x,d)sol_Handle(x,T1));
        plot_LFE(u_end-u_old,NewMesh); colorbar
       
        error(j,k) = L2Err_LFE(NewMesh,u_old,P7O6(),sol_Handle,T1);
    end %CFLS
end % mesh
...
sdata = size(data,1);
fid = fopen(['./results/LFE_RotHillscalarL2test_',locIntegrate,'_',Problem,'.txt'], 'wt');
