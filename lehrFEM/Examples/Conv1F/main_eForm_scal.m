...
% e formulation, scalar component
...
% model parameters
isigma = 10^-10;             % diffusion constant
mu = 1                  %
...
for i = 1:NREFS
    ...
    % source term
    L = assemLoad_LFE(NewMesh,P1O2(),d.Fs_Handle,isigma);   
    L_supg=assemLoad_LFE_SUPG(NewMesh,P1O2(),d.V_Handle,d.Fs_Handle,isigma,supg1,supg2);
    
    % Direchlet boundary
    [Es_up,FreeDofs] = assemDir_LFE(NewMesh,-1,d.Es_Handle,isigma);
    Es_st=Es_up;
    Es_supg=Es_up;
  
    L_up = -L - A_up*Es_up;
    L_st = -L - A_st*Es_st;
    L_supg = -(L+L_supg) - A_supg*Es_supg;

    % solving system
    Es_up(FreeDofs) = A_up(FreeDofs,FreeDofs)\L_up(FreeDofs);
    Es_st(FreeDofs) = A_st(FreeDofs,FreeDofs)\L_st(FreeDofs);
    Es_supg(FreeDofs) = A_supg(FreeDofs,FreeDofs)\L_supg(FreeDofs);

    % calulating L^2-error
    L2err(i,1) = L2Err_LFE(NewMesh,Es_up,P7O6(),d.Es_Handle,0,isigma);
    L2err(i,2) = L2Err_LFE(NewMesh,Es_st,P7O6(),d.Es_Handle,0,isigma);
    L2err(i,3) = L2Err_LFE(NewMesh,Es_supg,P7O6(),d.Es_Handle,0,isigma)
    
    % calculating H@1-semi -error
    H1Serr(i,1) = H1SErr_LFE(NewMesh,Es_up,P7O6(),d.GRAD_Es_Handle,0,isigma);
    H1Serr(i,2) = H1SErr_LFE(NewMesh,Es_st,P7O6(),d.GRAD_Es_Handle,0,isigma);
    H1Serr(i,3) = H1SErr_LFE(NewMesh,Es_supg,P7O6(),d.GRAD_Es_Handle,0,isigma);

    Dofs(i)=size(NewMesh.Coordinates,1);
     
    plot_LFE(Es_up,NewMesh); colorbar; title('Upwind');
    plot_LFE(Es_st,NewMesh);  colorbar; title('Standard');
    plot_LFE(Es_supg,NewMesh); colorbar;title('SUPG');
