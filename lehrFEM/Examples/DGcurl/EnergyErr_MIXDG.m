function err = EnergyErr_MIXDG(Mesh,u,QuadRule,UHandle1,UHandle2,Curl_UHandle,Div_UHandle,SIGMA1,SIGMA2,alpha1,alpha2,varargin)
%EnergyErr_MIXDG DG energy-norm of discretization error of the discontinuous galerkin solution
% 
%   ERR = ENERGYERR_MIXDG(Mesh,u,QuadRule,UHANDLE1,UHANDLE2,Curl_UHANDLE,Div_UHANDLE,SIGMA1,SIGMA2,alpha1,alpha2,varargin)
...
%   CH-8092 Zurich, Switzerland
  ...
  for i = 1:nElements
    ...
    Div_u_FE = u(idx(1))*dND(1) + u(idx(2))*dND(2) + u(idx(3))*dND(3) + u(idx(4))*dND(4) + u(idx(5))*dND(5) + u(idx(6))*dND(6);
    Div_u_EX = Div_UHandle(x,varargin{:});
    
    % Compute error on current element
    err = err+sum(QuadRule.w.*abs(u_EX1-u_FE1).^2)*det_BK+sum(QuadRule.w.*abs(u_EX2-u_FE2).^2)*det_BK+sum(QuadRule.w.*abs(Curl_u_EX-Curl_u_FE).^2)*det_BK+sum(h^2*QuadRule.w.*abs(Div_u_EX-Div_u_FE).^2)*det_BK;
